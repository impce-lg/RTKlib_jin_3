 ...
 *         27 August, 1998
 *     [3] D.D.McCarthy, IERS Technical Note 21, IERS Conventions 1996, July
 *1996 [4] D.A.Vallado, Fundamentals of Astrodynamics and Applications 2nd ed,
...
static int addpeph(nav_t *nav, peph_t *peph) {
  ...
  if (nav->ne >= nav->nemax) {
    nav->nemax += 256;
    if (!(nav_peph =
              (peph_t *)realloc(nav->peph, sizeof(peph_t) * nav->nemax))) {
...
static void readsp3b(FILE *fp, char type, int *sats, int ns, double *bfact,
  ...
  while (fgets(buff, sizeof(buff), fp)) {
    ...
    for (i = pred_o = pred_c = v = 0; i < n && fgets(buff, sizeof(buff), fp);
         i++) {
      ...
      for (j = 0; j < 4; j++) {
        ...
        if (buff[0] == 'P') { /* position */
          ...
          if ((base = bfact[j < 3 ? 0 : 1]) > 0.0 && std > 0.0) {
            peph.std[sat - 1][j] =
                (float)(pow(base, std) * (j < 3 ? 1E-3 : 1E-12));
          }
        } else if (v) { /* velocity */
          if (val != 0.0 && fabs(val - 999999.999999) >= 1E-6) {
          ...
          if ((base = bfact[j < 3 ? 0 : 1]) > 0.0 && std > 0.0) {
            peph.vst[sat - 1][j] =
                (float)(pow(base, std) * (j < 3 ? 1E-7 : 1E-16));
          }
        }
      }
    }
 ...
 *+ 4: not combined) return : none notes  : see ref [1] precise ephemeris is
 *appended and combined nav->peph and nav->ne must by properly initialized
 *before calling the function only files with extensions of .sp3, .SP3, .eph*
 *and .EPH* are read
 *-----------------------------------------------------------------------------*/
...
/* read dcb parameters file (P1C1**.DCB) -------------------------------------*/
...
/* read dcb parameters body buff of **_DCB.BSX version 0.01 ------------------*/
static int readdcbbuff_bsx001(const char *buff, nav_t *nav, const sta_t *sta) {
  double cbias;
  char strSysPRN[3], strSiteName[4] = "", strObs1[4], strObs2[4];
  int i, j, sat, type = 0;

  if ((cbias = str2num(buff, 71, 21)) == 0.0) return 0;

  if ((strstr(buff, "C1W") && strstr(buff, "C2W")) ||
      (strstr(buff, "C1W") && strstr(buff, "C2P")) ||
      (strstr(buff, "C1P") && strstr(buff, "C2W")) ||
      (strstr(buff, "C1P") && strstr(buff, "C2P")))
    type = 1;
  else if ((strstr(buff, "C1C") && strstr(buff, "C1W")) ||
           (strstr(buff, "C1C") && strstr(buff, "C1P")))
    type = 2;
  else if ((strstr(buff, "C2C") && strstr(buff, "C2W")) ||
           (strstr(buff, "C2C") && strstr(buff, "C2P")))
    type = 3;
  else
    return 0;

  for (i = 0; i < 3; i++) {
    strSysPRN[i] = buff[i + 11];
    strObs1[i] = buff[i + 30];
    strObs2[i] = buff[i + 35];
  }
  for (i = 0; i < 4; i++) strSiteName[i] = buff[i + 15];

  if (sta &&
      (!strcmp(strSysPRN, "G") || !strcmp(strSysPRN, "R") ||
       !strcmp(strSysPRN, "E") || !strcmp(strSysPRN, "C"))) { /* receiver dcb */
    for (i = 0; i < MAXRCV; i++) {
      if (!strcmp(sta[i].name, strSiteName)) break;
    }
    if (i < MAXRCV) {
      j = !strcmp(strSysPRN, "G")
              ? 0
              : (!strcmp(strSysPRN, "R") ? 1
                                         : (!strcmp(strSysPRN, "E") ? 2 : 3));
      nav->rbias[i][j][type - 1] = cbias * 1E-9 * CLIGHT; /* ns -> m */
    }
  } else if ((sat = satid2no(strSysPRN))) {                /* satellite dcb */
    nav->cbias[sat - 1][type - 1] = cbias * 1E-9 * CLIGHT; /* ns -> m */
    if (strObs1[2] == 'C') {
      nav->cbias[sat - 1][type - 1] = -nav->cbias[sat - 1][type - 1];
    }
  }

  return 1;
}
/* read dcb parameters body buff of **_DCB.BSX version 1.00 ------------------*/
static int readdcbbuff_bsx100(const char *buff, nav_t *nav, const sta_t *sta) {
  double cbias;
  char strSysPRN[3], strSiteName[4] = "", strObs1[4], strObs2[4];
  int i, j, sat, type = 0;

  if ((cbias = str2num(buff, 70, 21)) == 0.0) return 0;
  ...
  for (i = 0; i < 9; i++) strSiteName[i] = buff[i + 15];
...
/* read dcb parameters file (**_DCB.BSX) -------------------------------------*/
static int readdcbf_bsx(const char *file, nav_t *nav, const sta_t *sta) {
  FILE *fp;
  double version;
  char buff[256];

  trace(3, "readdcbf_bsx: file=%s\n", file);
  ...
  if (version == 0.01) {
    while (fgets(buff, sizeof(buff), fp)) {
      ...
      if (!strstr(buff, "DCB")) continue;
      readdcbbuff_bsx001(buff, nav, sta);
    }
  } else if (version == 1.00) {
    while (fgets(buff, sizeof(buff), fp)) {
      // DSB: Differential Signal Bias
      if (!strstr(buff, "DSB")) continue;
      readdcbbuff_bsx100(buff, nav, sta);
    }
  } else
    return 0;
  fclose(fp);
...
extern int readdcb(const char *file, nav_t *nav, const sta_t *sta) {
  ...
  for (i = 0; i < n; i++) {
    if (strstr(efiles[i], "BSX"))
      readdcbf_bsx(efiles[i], nav, sta);
    else
...
/* add satellite fcb ---------------------------------------------------------*/
static int addfcb(nav_t *nav, gtime_t sGTimeStart, gtime_t sGTimeEnd, int sat,
                  const double *bias, const double *std) {
  fcbd_t *nav_fcb;
  int i, j;

  if (nav->nf > 0 &&
      fabs(timediff(sGTimeStart, nav->fcb[nav->nf - 1].sGTimeStart)) <= 1e-3) {
    for (i = 0; i < 3; i++) {
  ...
  nav->fcb[nav->nf].sGTimeStart = sGTimeStart;
  nav->fcb[nav->nf++].sGTimeEnd = sGTimeEnd;
  return 1;
}
...
static int readfcbf(const char *file, nav_t *nav) {
  ...
  while (fgets(buff, sizeof(buff), fp)) {
    ...
    if (sscanf(buff,
               ...
               "%lf %lf %lf %lf %lf %lf",
               ep1, ep1 + 1, ep1 + 2, ep1 + 3, ep1 + 4, ep1 + 5, ep2, ep2 + 1,
               ep2 + 2, ep2 + 3, ep2 + 4, ep2 + 5, str, bias, std, bias + 1,
               std + 1, bias + 2, std + 2) < 17)
      continue;
    if (!(sat = satid2no(str))) continue;
    sGTimeStart = epoch2time(ep1);
    sGTimeEnd = epoch2time(ep2);
    if (!addfcb(nav, sGTimeStart, sGTimeEnd, sat, bias, std)) return 0;
  }
  fclose(fp);
  return 1;
}
...
static int cmpfcb(const void *p1, const void *p2) {
  ...
  double tt = timediff(q1->sGTimeStart, q2->sGTimeStart);
...
static int pephclk(gtime_t time, int sat, const nav_t *nav, double *dts,
  ...
  if (t[0] <= 0.0) {
    ...
    i = t[0] < -t[1] ? 0 : 1;
    std =
        nav->pclk[index + i].std[sat - 1][0] * CLIGHT + EXTERR_CLK * fabs(t[i]);
  } else {
...
extern void satantoff(gtime_t time, const double *rs, int sat, const nav_t *nav,
  ...
  /* iono-free LC */
  for (i = 0; i < 3; i++) {
    dant1 = pcv->off[j][0] * ex[i] + pcv->off[j][1] * ey[i] +
            pcv->off[j][2] * ez[i];
    dant2 = pcv->off[k][0] * ex[i] + pcv->off[k][1] * ey[i] +
            pcv->off[k][2] * ez[i];
 ...
 * notes  : clock includes relativistic correction but does not contain code
 *bias before calling the function, nav->peph, nav->ne, nav->pclk and nav->nc
 *must be set by calling readsp3(), readrnx() or readrnxt() if precise clocks
 *are not set, clocks in sp3 are used instead
 *-----------------------------------------------------------------------------*/
extern int peph2pos(gtime_t time, int sat, const nav_t *nav, int opt,
                    double *rs, double *dts, double *var) {
  gtime_t time_tt;
  double rss[3], rst[3], dtss[1], dtst[1], dant[3] = {0}, vare = 0.0,
                                           varc = 0.0, tt = 1E-3;
  int i;
  ...
  if (!pephpos(time_tt, sat, nav, rst, dtst, NULL, NULL) ||
      !pephclk(time_tt, sat, nav, dtst, NULL))
    return 0;
