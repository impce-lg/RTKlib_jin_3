...
static void genpath(const char *file, const char *name, gtime_t time, int seqno,
  ...
  for (p = buff, q = (char *)file; (*p = *q); p++, q++) {
    if (*q == '%')
      ...
      continue;
    if (*q == 's' || *q == 'r')
      p += sprintf(p, "%s", l_name) - 1;
    else if (*q == 'S' || *q == 'R')
      p += sprintf(p, "%s", u_name) - 1;
    else if (*q == 'N')
      p += sprintf(p, "%d", seqno) - 1;
...
static int exec_down(const path_t *path, char *remot_p, const char *usr,
  ...
  /* proxy option */
  if (*proxy) {
    sprintf(env, "set %s_proxy=http://%s & ", proto == 0 ? "ftp" : "http",
            proxy);
  ...
  if (proto == 0) {
    ...
    sprintf(cmd, "%s%s %s %s-t %d -T %d -O \"%s\" -o \"%s\"%s\n", env, FTP_CMD,
            path->remot, opt, FTP_RETRY, FTP_TIMEOUT, path->local, errfile,
            opt2);
  }
...
static int test_locals(gtime_t ts, gtime_t te, double ti, const url_t *url,
  ...
  if (strstr(url->path, "%s") || strstr(url->path, "%S")) {
    ...
    for (i = 0; i < nsta; i++) {
      ...
      if (test_local(ts, te, ti, url->path, stas[i], *dir ? dir : url->dir,
                     nc + i, nt + i, fp)) {
        return 1;
      }
    }
    ...
    if (test_local(ts, te, ti, url->path, "", *dir ? dir : url->dir, nc, nt,
                   fp)) {
      return 1;
    }
  }
 ...
 *        follwing keywords replaced by date, time, station names and
...
extern int dl_exec(gtime_t ts, gtime_t te, double ti, int seqnos, int seqnoe,
  ...
  while (timediff(ts, te) < 1E-3) {
    for (i = 0; i < nurl; i++) {
      if (!gen_paths(ts, ts_p, seqnos, seqnoe, urls + i, stas, nsta, dir,
                     &paths)) {
        free_path(&paths);
        return 0;
      }
  ...
  for (i = 0; i < paths.n; i++) {
    sprintf(str, "%s->%s (%d/%d)", paths.path[i].remot, paths.path[i].local,
            i + 1, paths.n);
  ...
  sprintf(msg, "OK=%d No_File=%d Skip=%d Error=%d (Time=%.1f s)", n[0], n[1],
          n[2], n[3], (tickget() - tick) * 0.001);
  ...
  for (; timediff(ts, te) < 1E-3 && !abort; ts = timeadd(ts, ti * ncol)) {
    ...
    *date_p = '\0';
    ...
    for (i = j = 0; i < nurl && !abort; i++) {
      ...
      /* test local files */
      abort = test_locals(ts, time, ti, urls + i, stas, nsta, dir, nc + j,
                          nt + j, fp);
