...
static nav_t sNavData = {0};     /* navigation data */
...
static void outheader(FILE *fp, char **file, int NumInFiles,
  ...
  if (!sSolutionOption->outhead || sSolutionOption->posf == SOLF_NMEA ||
  ...
  if (!revs) { /* input forward data */
    ...
    while (isbs < sbss.n) {
      ...
      if (getbitu(sbss.msgs[isbs].msg, 8, 6) != 9) { /* except for geo nav */
        sbsupdatecorr(sbss.msgs + isbs, &sNavData);
      }
    ...
    while (ilex < lexs.n) {
      if (lexupdatecorr(lexs.msgs + ilex, &sNavData, &time)) {
        if (timediff(time, obs[0].time) > -1.0 - DTTOL) break;
      }
      ilex++;
    }
...
static void procpos(FILE *fp, const prcopt_t *sProcessOption,
  ...
  while ((nobs = inputobs(obs, rtk.sol.stat, sProcessOption)) >= 0) {
    ...
    if (sNavData.nf > 0) {
      corr_phase_bias_fcb(obs, n, &sNavData);
    } else if (!strstr(sProcessOption->pppopt, "-DIS_FCB")) {
      corr_phase_bias_ssr(obs, n, &sNavData);
    }
    /* disable L2 */
#if 0
...
#endif
    if (!rtkpos(&rtk, obs, n, &sNavData)) continue;
...
static int readobsnav(gtime_t sGTimeStart, gtime_t sGTimeEnd,
  ...
  if (sGTimeStart.time == 0 || sGTimeEnd.time == 0) {
    ...
    if (i < j) {
      ...
      settspan(sGTimeStart, sGTimeEnd);
    }
  }
  return 1;
}
...
static int openses(const prcopt_t *sProcessOption,
  ...
  if (*sFileOption->satantp && !(readpcv(sFileOption->satantp, pcvs))) {
    showmsg("error : no sat ant pcv in %s", sFileOption->satantp);
  ...
  if (*sFileOption->rcvantp && !(readpcv(sFileOption->rcvantp, pcvr))) {
    showmsg("error : no rec ant pcv in %s", sFileOption->rcvantp);
  ...
  if (sSolutionOption->geoid > 0 && *sFileOption->geoid) {
    if (!opengeoid(sSolutionOption->geoid, sFileOption->geoid)) {
      showmsg("error : no geoid data %s", sFileOption->geoid);
...
static int outhead(const char *archOutFiles, char **archInFiles, int NumInFiles,
  ...
  if (*archOutFiles) {
    ...
    if (!(fp = fopen(archOutFiles, "w"))) {
      showmsg("error : open output file %s", archOutFiles);
      return 0;
    }
  }
  /* output header */
...
static int execses(gtime_t sGTimeStart, gtime_t sGTimeEnd, double PrcsInterval,
  ...
  if (*sFileOption->iono && (ext = strrchr(sFileOption->iono, '.'))) {
    if (strlen(ext) == 4 && (ext[3] == 'i' || ext[3] == 'I')) {
      ...
      readtec(path, &sNavData, 1);
    }
  }
  /* read erp data */
  if (*sFileOption->eop) {
    free(sNavData.erp.data);
    sNavData.erp.data = NULL;
    sNavData.erp.n = sNavData.erp.nmax = 0;
    reppath(sFileOption->eop, path, sGTimeStart, "", "");
    if (!readerp(path, &sNavData.erp)) {
  ...
  if (!readobsnav(sGTimeStart, sGTimeEnd, PrcsInterval, archInFiles,
                  arIndex_InFiles, NumInFiles, &popt_, &obss, &sNavData, stas))
    return 0;

  /* read dcb parameters */
  if (*sFileOption->dcb) {
    reppath(sFileOption->dcb, path, sGTimeStart, "", "");
    readdcb(path, &sNavData, stas);
  }
  /* set antenna paramters */
  if (popt_.mode != PMODE_SINGLE) {
    setpcv(obss.n > 0 ? obss.data[0].time : timeget(), &popt_, &sNavData,
  ...
  if (popt_.mode == PMODE_FIXED) {
    if (!antpos(&popt_, 1, &obss, &sNavData, stas, sFileOption->stapos)) {
      freeobsnav(&obss, &sNavData);
      return 0;
    }
  } else if (PMODE_DGPS <= popt_.mode && popt_.mode <= PMODE_STATIC) {
    if (!antpos(&popt_, 2, &obss, &sNavData, stas, sFileOption->stapos)) {
      freeobsnav(&obss, &sNavData);
      return 0;
    }
  }
  ...
  archOutFiles = strcat(posfile, ".txt");
  /* write header to output file */
  ...
  /* read ground truth file (if exists) */
  if (sFileOption->groundtruth[0] != '\0') {
    // char gtfile[100];
    reppath(sFileOption->groundtruth, path, sGTimeStart, "", "");
    readgt(path);
  }
...
static int execses_b(gtime_t sGTimeStart, gtime_t sGTimeEnd,
  ...
  readpreceph(archInFiles, NumInFiles, sProcessOption, &sNavData, &sbss, &lexs);
  ...
  if (i < NumInFiles) { /* include base station keywords */
    if (!(base_ = (char *)malloc(strlen(pchBase) + 1))) {
      freepreceph(&sNavData, &sbss, &lexs);
      return 0;
    }
...
extern int postpos(gtime_t sGTimeStart, gtime_t sGTimeEnd, double PrcsInterval,
  ...
  if (!openses(sProcessOption, sSolutionOption, sFileOption, &sNavData,
  ...
  if (sGTimeStart.time != 0 && sGTimeEnd.time != 0 && PrcsUnitTime >= 0.0) {
    if (timediff(sGTimeEnd, sGTimeStart) < 0.0) {
      ...
      closeses(&sNavData, &RecAntParas, &SatAntParas);
      return 0;
    }
