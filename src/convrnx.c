...
static void convcode(double ver, int sys, char *type) {
  if (ver >= 2.12 && (sys == SYS_GPS || sys == SYS_QZS || sys == SYS_SBS) &&
    ...
    strcpy(type + 1, "C");
  } else if (ver >= 2.12 && sys == SYS_GLO &&
             !strcmp(type + 1, "1C")) { /* L1C/A */
    strcpy(type + 1, "A");
  } else if (ver >= 2.12 && sys == SYS_GLO &&
             !strcmp(type + 1, "2C")) { /* L2C/A */
    strcpy(type + 1, "D");
  } else if (sys == SYS_CMP &&
             (!strcmp(type + 1, "1I") || !strcmp(type + 1, "1Q") ||
              !strcmp(type + 1, "1X"))) { /* B1 */
    strcpy(type + 1, "2");
  } else if (!strcmp(type, "C1P") || !strcmp(type, "C1W") ||
             !strcmp(type, "C1Y") || !strcmp(type, "C1N")) { /* L1P,P(Y) */
    strcpy(type, "P1");
  } else if (!strcmp(type, "C2P") || !strcmp(type, "C2W") ||
             !strcmp(type, "C2Y") || !strcmp(type, "C2N") ||
             !strcmp(type, "C2D")) { /* L2P,P(Y) */
    strcpy(type, "P2");
  } else {
    type[2] = '\0';
  }
...
static int scan_obstype(int format, const char *file, rnxopt_t *opt,
  ...
  while ((type = input_strfile(str)) >= -1) {
    ...
    if (!opt->sGTimeStart.time ||
        timediff(str->obs->data[0].time, opt->sGTimeStart) >= 0.001) {
    ...
    if (opt->sGTimeEnd.time &&
        timediff(str->obs->data[0].time, opt->sGTimeEnd) > 10.0)
      break;
  ...
  for (i = 0; i < 7; i++)
    for (j = 0; j < n[i]; j++) {
      trace(2, "scan_obstype: sys=%d code=%s type=%d\n", i,
            code2obs(codes[i][j], NULL), types[i][j]);
    }
  for (i = 0; i < 7; i++) {
    /* sort codes */
...
static void set_obstype(int format, rnxopt_t *opt) {
  ...
  const unsigned char codes_oem3[7][8] = {
      /* novatel oem3 */
      {CODE_L1C, CODE_L2P}, {0}, {0}, {0}, {CODE_L1C}, {0}, {0}};
  ...
  const unsigned char codes_rinex[7][32] = {
      /* rinex and binex */
      {CODE_L1C, CODE_L1P, CODE_L1W, CODE_L1Y, CODE_L1M, CODE_L1N, CODE_L1S,
       CODE_L1L, CODE_L2C, CODE_L2D, CODE_L2S, CODE_L2L, CODE_L2X, CODE_L2P,
       CODE_L2W, CODE_L2Y, CODE_L2M, CODE_L2N, CODE_L5I, CODE_L5Q, CODE_L5X},
      {CODE_L1C, CODE_L1P, CODE_L2C, CODE_L2P, CODE_L3I, CODE_L3Q, CODE_L3X},
      {CODE_L1C, CODE_L1A, CODE_L1B, CODE_L1X, CODE_L1Z, CODE_L5I, CODE_L5Q,
       CODE_L5X, CODE_L6A, CODE_L6B, CODE_L6C, CODE_L6X, CODE_L6Z, CODE_L7I,
       CODE_L7Q, CODE_L7X, CODE_L8I, CODE_L8Q, CODE_L8X},
      {CODE_L1C, CODE_L1S, CODE_L1L, CODE_L1X, CODE_L1Z, CODE_L2S, CODE_L2L,
       CODE_L2X, CODE_L5I, CODE_L5Q, CODE_L5X, CODE_L6S, CODE_L6L, CODE_L6X},
      {CODE_L1C, CODE_L5I, CODE_L5Q, CODE_L5X},
      {CODE_L1I, CODE_L1Q, CODE_L1X, CODE_L7I, CODE_L7Q, CODE_L7X, CODE_L6I,
       CODE_L6Q, CODE_L6X},
      {CODE_L5A, CODE_L5B, CODE_L5C, CODE_L5X, CODE_L9A, CODE_L9B, CODE_L9C,
       CODE_L9X}};
  ...
  const unsigned char codes_other[7][8] = {/* others */
                                           {CODE_L1C}, {CODE_L1C}, {CODE_L1C},
                                           {CODE_L1C}, {CODE_L1C}, {CODE_L1I},
                                           {0}};
...
static void saveslips(unsigned char slips[][NFREQ + NEXOBS], obsd_t *data,
                      int n) {
  int i, j;
...
static void restslips(unsigned char slips[][NFREQ + NEXOBS], obsd_t *data,
                      int n) {
  int i, j;
...
static void convobs(FILE **ofp, rnxopt_t *opt, strfile_t *str, int *n,
  ...
  if (!screent(time, opt->sGTimeStart, opt->sGTimeEnd, opt->tint)) return;
...
static void convnav(FILE **ofp, rnxopt_t *opt, strfile_t *str, int *n) {
  ...
  ts1 = opt->sGTimeStart;
  if (ts1.time != 0) ts1 = timeadd(ts1, -MAXDTOE);
  te1 = opt->sGTimeEnd;
  if (te1.time != 0) te1 = timeadd(te1, MAXDTOE);
  ts2 = opt->sGTimeStart;
  if (ts2.time != 0) ts2 = timeadd(ts2, -MAXDTOE_GLO);
  te2 = opt->sGTimeEnd;
  if (te2.time != 0) te2 = timeadd(te2, MAXDTOE_GLO);

  sys = satsys(str->sat, &prn) & opt->navsys;

  if (sys == SYS_GPS) {
    if (opt->exsats[str->sat - 1] == 1 || !screent(str->time, ts1, te1, 0.0))
      return;
    ...
    if (opt->exsats[str->sat - 1] == 1 || !screent(str->time, ts2, te2, 0.0))
      return;
...
static void convsbs(FILE **ofp, rnxopt_t *opt, strfile_t *str, int *n) {
  ...
  if (ofp[6]) { /* output sbas log */
    if (screent(gpst2time(str->raw.sbsmsg.week, str->raw.sbsmsg.tow),
                opt->sGTimeStart, opt->sGTimeEnd, 0.0)) {
      sbsoutmsg(ofp[6], &str->raw.sbsmsg);
      n[6]++;
    }
  }
  if (!(opt->navsys & SYS_SBS) || msg != 9 ||
      !screent(str->time, ts1, te1, 0.0))
    return;
...
static void convlex(FILE **ofp, rnxopt_t *opt, strfile_t *str, int *n) {
  ...
  if (ofp[6] && screent(str->time, opt->sGTimeStart, opt->sGTimeEnd, 0.0)) {
    lexoutmsg(ofp[6], &str->raw.lexmsg);
    n[6]++;
  }
}
/* set approx position -------------------------------------------------------*/
...
/* show status message -------------------------------------------------------*/
static int showstat(int sess, gtime_t sGTimeStart, gtime_t sGTimeEnd, int *n) {
  ...
  if (sGTimeStart.time != 0) {
    time2str(sGTimeStart, s, 0);
    p += sprintf(p, "%s", s);
  }
  if (sGTimeEnd.time != 0 && timediff(sGTimeEnd, sGTimeStart) > 0.9) {
    time2str(sGTimeEnd, s, 0);
    p += sprintf(p, "-%s", s + 5);
  }
  p += sprintf(p, ": ");
...
static int convrnx_s(int sess, int format, rnxopt_t *opt, const char *file,
  ...
  strfile_t *str;
  gtime_t sGTimeStart = {0}, sGTimeEnd = {0}, tend = {0}, time = {0};
  ...
  if (reppath(file, path, opt->sGTimeStart, staid, "") < 0) {
  ...
  time = opt->sGTimeStart.time
             ? opt->sGTimeStart
             : (time.time ? timeadd(time, TSTARTMARGIN) : time);
  ...
  for (i = 0; i < nf && !abort; i++) {
    ...
    for (j = 0; (type = input_strfile(str)) >= -1; j++) {
      if (j % 11 == 1 && (abort = showstat(sess, sGTimeEnd, sGTimeEnd, n)))
        break;
      ...
      switch (type) {
        case 1:
          convobs(ofp, opt, str, n, slips);
          break;
        case 2:
          convnav(ofp, opt, str, n);
          break;
        case 3:
          convsbs(ofp, opt, str, n);
          break;
        case 31:
          convlex(ofp, opt, str, n);
          break;
        case -1:
          n[NOUTFILE]++;
          break; /* error */
      ...
      sGTimeEnd = str->time;
      if (sGTimeStart.time == 0) sGTimeStart = sGTimeEnd;

      /* set approx position */
      if (type == 1 && !opt->autopos && norm(opt->apppos, 3) <= 0.0) {
        setapppos(str, opt);
      }
      if (opt->sGTimeEnd.time && timediff(sGTimeEnd, opt->sGTimeEnd) > 10.0)
        break;
    }
    /* close stream file */
    close_strfile(str);

    tend = sGTimeEnd; /* end time of a file */
  }
  ...
  if (sGTimeStart.time > 0) showstat(sess, sGTimeStart, sGTimeEnd, n);

  for (i = 0; i < MAXEXFILE; i++) free(epath[i]);

  free_strfile(str);

  if (opt->tstart.time == 0) opt->tstart = opt->sGTimeStart;
  if (opt->tend.time == 0) opt->tend = opt->sGTimeEnd;
...
extern int convrnx(int format, rnxopt_t *opt, const char *file, char **ofile) {
  gtime_t t0 = {0};
  rnxopt_t opt_ = *opt;
  double tu, sGTimeStart;
  int i, week, stat = 1;

  trace(3, "convrnx: format=%d file=%s ofile=%s %s %s %s %s %s %s\n", format,
        file, ofile[0], ofile[1], ofile[2], ofile[3], ofile[4], ofile[5],
        ofile[6]);

  showmsg("");

  if (opt->sGTimeStart.time == 0 || opt->sGTimeEnd.time == 0 ||
      opt->tunit <= 0.0) {
    /* single-session */
    opt_.tstart = opt_.tend = t0;
    stat = convrnx_s(0, format, &opt_, file, ofile);
  } else if (timediff(opt->sGTimeStart, opt->sGTimeEnd) <= 0.0) {
    /* multiple-session */
    tu = opt->tunit < 86400.0 ? opt->tunit : 86400.0;
    sGTimeStart = tu * (int)floor(time2gpst(opt->sGTimeStart, &week) / tu);

    for (i = 0;; i++) { /* for each session */
      opt_.sGTimeStart = gpst2time(week, sGTimeStart + i * tu);
      opt_.sGTimeEnd = timeadd(opt_.sGTimeStart, tu - DTTOL - 0.001);
      opt_.trtcm =
          timeadd(opt->trtcm, timediff(opt_.sGTimeStart, opt->sGTimeStart));
      if (timediff(opt_.sGTimeStart, opt->sGTimeEnd) > 0.0) break;

      if (timediff(opt_.sGTimeStart, opt->sGTimeStart) < 0.0)
        opt_.sGTimeStart = opt->sGTimeStart;
      if (timediff(opt_.sGTimeEnd, opt->sGTimeEnd) > 0.0)
        opt_.sGTimeEnd = opt->sGTimeEnd;
      opt_.tstart = opt_.tend = t0;
