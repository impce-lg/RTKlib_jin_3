 ...
 *                           change api of
 *outrnxobsh(),outrnxobsb(),outrnxnavb() 2009/06/02 1.8  add api outrnxgnavb()
...
static int sat2code(int sat, char *code) {
  ...
  switch (satsys(sat, &prn)) {
    case SYS_GPS:
      sprintf(code, "G%2d", prn - MINPRNGPS + 1);
      break;
    case SYS_GLO:
      sprintf(code, "R%2d", prn - MINPRNGLO + 1);
      break;
    case SYS_GAL:
      sprintf(code, "E%2d", prn - MINPRNGAL + 1);
      break;
    case SYS_SBS:
      sprintf(code, "S%2d", prn - 100);
      break;
    case SYS_QZS:
      sprintf(code, "J%2d", prn - MINPRNQZS + 1);
      break;
    case SYS_CMP:
      sprintf(code, "C%2d", prn - MINPRNCMP + 1);
      break;
    case SYS_IRN:
      sprintf(code, "I%2d", prn - MINPRNIRN + 1);
      break;
    default:
      return 0;
  }
...
static void convcode(double ver, int sys, const char *str, char *type) {
  ...
  if (!strcmp(str, "P1")) { /* ver.2.11 GPS L1PY,GLO L2P */
    ...
    if (sys == SYS_GPS) {
      ...
      sprintf(type, "%c1X", 'C');            /* ver.2.12 B1 */
    ...
    if (sys == SYS_GPS)
      ...
      sprintf(type, "%c1X", str[0]); /* extension */
    ...
    if (sys == SYS_GPS)
      ...
      sprintf(type, "%c1X", str[0]); /* ver.2.12 B1 */
  } else if (str[1] == '5') {
    if (sys == SYS_GPS)
    ...
    if (sys == SYS_GAL)
      ...
      sprintf(type, "%c7X", str[0]); /* ver.2.12 B2 */
  } else if (str[1] == '8') {
...
static void set_index(double ver, int sys, const char *opt,
  ...
  for (i = 0; i < NFREQ; i++) {
    for (j = 0, k = -1; j < n; j++) {
      if (ind->frq[j] == i + 1 && ind->pri[j] &&
          (k < 0 || ind->pri[j] > ind->pri[k])) {
        k = j;
      }
    }
    if (k < 0) continue;
...
static int decode_geph(double ver, int sat, gtime_t toc, double *data,
  ...
  *geph = geph0;
  ...
  tod = ver <= 2.99 ? data[2]
                    : fmod(data[2], 86400.0); /* tod (v.2), tow (v.3) in utc */
...
static int add_geph(nav_t *nav, const geph_t *geph) {
  ...
  if (nav->ngmax <= nav->ng) {
    nav->ngmax += 1024;
    if (!(nav_geph =
              (geph_t *)realloc(nav->geph, sizeof(geph_t) * nav->ngmax))) {
...
static int add_seph(nav_t *nav, const seph_t *seph) {
  ...
  if (nav->nsmax <= nav->ns) {
    nav->nsmax += 1024;
    if (!(nav_seph =
              (seph_t *)realloc(nav->seph, sizeof(seph_t) * nav->nsmax))) {
...
static int readrnxnav(FILE *fp, const char *opt, double ver, int sys,
  ...
  /* read rinex navigation data body */
  while ((stat = readrnxnavb(fp, opt, ver, sys, &type, &eph, &geph, &seph)) >=
         0) {
    /* add ephemeris to navigation data */
    if (stat) {
      switch (type) {
...
static int readrnxclk(FILE *fp, const char *opt, int index, nav_t *nav) {
  ...
  while (fgets(buff, sizeof(buff), fp)) {
    ...
    if (nav->nc >= nav->ncmax) {
      nav->ncmax += 1024;
      if (!(nav_pclk =
                (pclk_t *)realloc(nav->pclk, sizeof(pclk_t) * (nav->ncmax)))) {
...
static int readrnxfp(FILE *fp, gtime_t ts, gtime_t te, double tint,
  ...
  switch (*type) {
    case 'O':
      return readrnxobs(fp, ts, te, tint, opt, index, ver, tsys, tobs, obs);
    case 'N':
    ...
    case 'C':
      return readrnxclk(fp, opt, index, nav);
  }
 ...
 *
 *            -GLss[=shift]: select GPS signal ss (ss: RINEX 3 code,
 *"1C","2W"...) -RLss[=shift]: select GLO signal ss -ELss[=shift]: select GAL
 *signal ss -JLss[=shift]: select QZS signal ss -CLss[=shift]: select BDS signal
 *ss -ILss[=shift]: select IRN signal ss -SLss[=shift]: select SBS signal ss
 *
...
extern int readrnxt(const char *file, int rcv, gtime_t ts, gtime_t te,
  ...
  for (i = 0; i < n && stat >= 0; i++) {
    stat =
        readrnxfile(files[i], ts, te, tint, opt, 0, rcv, &type, obs, nav, sta);
  }
...
extern int readrnxc(const char *file, nav_t *nav) {
  ...
  /* read rinex clock files */
  for (i = 0; i < n; i++) {
    if (readrnxfile(files[i], t, t, 0.0, "", 1, index++, &type, NULL, nav,
                    NULL)) {
      continue;
    }
    stat = 0;
...
extern int init_rnxctr(rnxctr_t *rnx) {
  ...
  rnx->sys = rnx->tsys = 0;
  for (i = 0; i < 6; i++)
    for (j = 0; j < MAXOBSTYPE; j++) rnx->tobs[i][j][0] = '\0';
  rnx->obs.n = 0;
  rnx->nav.n = MAXSAT;
  rnx->nav.ng = NSATGLO;
  rnx->nav.ns = NSATSBS;
...
extern int input_rnxctr(rnxctr_t *rnx, FILE *fp) {
  ...
  if ((stat = readrnxnavb(fp, rnx->opt, rnx->ver, sys, &type, &eph, &geph,
                          &seph)) <= 0) {
    return stat < 0 ? -2 : 0;
  }
  if (type == 1) {
...
extern int outrnxobsh(FILE *fp, const rnxopt_t *opt, const nav_t *nav) {
  ...
  for (i = 0; i < 3; i++)
    if (fabs(opt->apppos[i]) < 1E8) pos[i] = opt->apppos[i];
  for (i = 0; i < 3; i++)
    if (fabs(opt->antdel[i]) < 1E8) del[i] = opt->antdel[i];
...
static int obsindex(double ver, int sys, const unsigned char *code,
  ...
  for (i = 0; i < NFREQ + NEXOBS; i++) {
    ...
    if (ver <= 2.99) { /* ver.2 */
      if (!strcmp(tobs, "C1") &&
          (sys == SYS_GPS || sys == SYS_GLO || sys == SYS_QZS ||
        ...
        if (code[i] == CODE_L2S || code[i] == CODE_L2L || code[i] == CODE_L2X)
          return i;
      } else if (ver >= 2.12 && tobs[1] == 'D' &&
                 sys == SYS_GLO) { /* GLO L2C/A */
        if (code[i] == CODE_L2C) return i;
...
extern int outrnxobsb(FILE *fp, const rnxopt_t *opt, const obsd_t *obs, int n,
  ...
  if (opt->rnxver <= 2.99) { /* ver.2 */
    fprintf(fp, " %02d %2.0f %2.0f %2.0f %2.0f%11.7f  %d%3d", (int)ep[0] % 100,
            ep[1], ep[2], ep[3], ep[4], ep[5], flag, ns);
    for (i = 0; i < ns; i++) {
    ...
    for (j = 0; j < opt->nobs[m]; j++) {
      ...
      switch (opt->tobs[m][j][0]) {
        ...
        case 'P':
          ...
          break;
        case 'L':
          outrnxobsf(fp, obs[ind[i]].L[k], obs[ind[i]].LLI[k]);
          break;
        case 'D':
          outrnxobsf(fp, obs[ind[i]].D[k], -1);
          break;
        case 'S':
          outrnxobsf(fp, obs[ind[i]].SNR[k] * 0.25, -1);
          break;
      }
    }
...
static void outnavf(FILE *fp, double value) {
  ...
  fprintf(fp, " %s.%012.0fE%+03.0f", value < 0.0 ? "-" : " ",
          fabs(value) / pow(10.0, e - 12.0), e);
}
    ...
    if ((opt->navsys & SYS_IRN) && opt->rnxver >= 3.03) { /* ver.3.03 */
      if (opt->outiono) {
        fprintf(fp, "IRNA %12.4E%12.4E%12.4E%12.4E%7s%-20s\n", nav->ion_irn[0],
                nav->ion_irn[1], nav->ion_irn[2], nav->ion_irn[3], "",
                "IONOSPHERIC CORR");
        fprintf(fp, "IRNB %12.4E%12.4E%12.4E%12.4E%7s%-20s\n", nav->ion_irn[4],
                nav->ion_irn[5], nav->ion_irn[6], nav->ion_irn[7], "",
                "IONOSPHERIC CORR");
      }
    }
    if (opt->navsys & SYS_GPS) {
...
extern int outrnxnavb(FILE *fp, const rnxopt_t *opt, const eph_t *eph) {
  ...
  if (opt->rnxver > 2.99 || sys == SYS_GAL ||
      sys == SYS_CMP) { /* ver.3 or ver.2 GAL */
