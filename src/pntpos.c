...
#define THRES_CDIF 10.0 /* threshold of code difference (abs(P1-P2),m) */
...
static double prange(const obsd_t *obs, const nav_t *nav, const double *azel,
  ...
  *var = 0.0;
  ...
  if (opt->ionoopt == IONOOPT_IFLC) { /* dual-frequency */
    ...
    if (fabs(P1 - P2) >= THRES_CDIF) return 0.0;
...
extern int ionocorr(gtime_t time, const nav_t *nav, int sat, const double *pos,
  ...
  trace(4, "ionocorr: time=%s opt=%d sat=%2d pos=%.3f %.3f azel=%.3f %.3f\n",
        time_str(time, 3), ionoopt, sat, pos[0] * R2D, pos[1] * R2D,
...
static int rescode(int iter, const obsd_t *obs, int n, const double *rs,
  ...
  for (i = *ns = 0; i < n && i < MAXOBS; i++) {
    ...
    /*if (iter == 5)
    {
            double Pv = varerr(opt, azel[1 + i * 2], sys);
            trace(6, "var[%d]:", nv); tracemat(6, var + nv-1, 1, 1, 5, 3);
            trace(6, "Pv:"); tracemat(6, &Pv, 1, 1, 5, 3);
            trace(6, "vare:"); tracemat(6, vare + i, 1, 1, 5, 3);
            trace(6, "vmeas"); tracemat(6, &vmeas, 1, 1, 5, 3);
            trace(6, "vion:"); tracemat(6, &vion, 1, 1, 5, 3);
            trace(6, "vtrp:"); tracemat(6, &vtrp, 1, 1, 5, 3);
            trace(6, "\n:");
    }*/
...
static int estpos(const obsd_t *obs, int n, const double *rs, const double *dts,
  ...
  int i, j, k, info, stat, nv, ns, sysn[NSYS] = {0};
  ...
  for (i = 0; i < MAXITR; i++) {
    /* pseudorange residuals */
    for (j = 0; j < NSYS; j++) sysn[j] = 0;
    nv = rescode(i, obs, n, rs, dts, vare, svh, nav, x, opt, v, H, var, azel,
                 vsat, resp, &ns, sysn);
...
/* estimate receiver position ,elevation task weight��v control---------------*/
static int estpos2(const obsd_t *obs, int n, const double *rs,
                   const double *dts, const double *vare, const int *svh,
                   const nav_t *nav, const prcopt_t *opt, sol_t *sol,
                   double *azel, int *vsat, double *resp, char *msg) {
  double x[NX] = {0}, dx[NX], Q[NX * NX], *v, *H, *var, *varscl, sig;
  int i, j, k, m, mm, info, stat, nv, ns, sysn[NSYS] = {0};
  ...
  for (i = 0; i < MAXITR; i++) {
    ...
    if (norm(dx, NX) < 1E-4) {
      ...
      if ((stat = valsol(azel, vsat, n, opt, v, nv, NX, msg))) {
        sol->type = 0;
        sol->time = timeadd(obs[0].time, -x[3] / CLIGHT);
        sol->dtr[0] = x[3] / CLIGHT; /* gps receiver clock bias (s) */
        sol->dtr[1] = x[4] / CLIGHT; /* glo-gps time offset (s) */
        sol->dtr[2] = x[5] / CLIGHT; /* gal-gps time offset (s) */
        sol->dtr[3] = x[6] / CLIGHT; /* bds-gps time offset (s) */
        for (j = 0; j < 6; j++) sol->rr[j] = j < 3 ? x[j] : 0.0;
...
static int raim_fde(const obsd_t *obs, int *ntemp, const double *rs,
                    const double *dts, const double *vare, const int *svh,
                    const nav_t *nav, const prcopt_t *opt, sol_t *sol,
                    double *azel, int *vsat, double *resp, char *msg) {
  int n = *ntemp;
  ...
  for (i = 0; i < n; i++) {
    ...
    if (!estpos(obs_e, n - 1, rs_e, dts_e, vare_e, svh_e, nav, opt, &sol_e,
...
static void estvel(const obsd_t *obs, int n, const double *rs,
 ...
 *receiver bias)
 *-----------------------------------------------------------------------------*/
extern int pntpos(const obsd_t *obs, int *ntemp, const nav_t *nav,
