...
static int is_navmsg(int msg) {
  return msg == 1019 || msg == 1020 || msg == 1044 || msg == 1045 ||
         msg == 1046 || msg == 1047 || msg == 63;
}
/* test station info message -------------------------------------------------*/
static int is_stamsg(int msg) {
  return msg == 1005 || msg == 1006 || msg == 1007 || msg == 1008 ||
         msg == 1033;
}
/* test time interval --------------------------------------------------------*/
...
/* convert stearm ------------------------------------------------------------*/
static void strconv(stream_t *str, strconv_t *conv, unsigned char *buff,
                    int n) {
  int i, ret;
...
extern int strsvrstart(strsvr_t *svr, int *opts, int *strs, char **paths,
  ...
  for (i = 0; i < svr->nstr; i++) {
    ...
    if ((p = strstr(file1, "::"))) *p = '\0';
    strcpy(file2, paths[i]);
    if ((p = strstr(file2, "::"))) *p = '\0';
