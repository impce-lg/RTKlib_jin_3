...
#define IT(r, opt) \
  (NP(opt) + NI(opt) + NT(opt) / 2 * (r)) /* tropos (r:0=rov,1:ref) */
#define IL(f, opt) (NP(opt) + NI(opt) + NT(opt) + (f)) /* receiver h/w bias */
#define IB(s, f, opt) \
  (NR(opt) + MAXSAT * (f) + (s)-1) /* phase bias (s:satno,f:freq) */
...
static int amblevel = 0;         /* rtk ambiguity output level (0:off) */
static FILE *fp_amb = NULL;      /* rtk ambiguity file pointer */
static char file_amb[1024] = ""; /* rtk ambiguity file original path */
static gtime_t time_amb = {0};   /* rtk ambiguity file time */

static int reslevel = 0;         /* rtk residual output level (0:off) */
static FILE *fp_res = NULL;      /* rtk residual file pointer */
static char file_res[1024] = ""; /* rtk residual file original path */
static gtime_t time_res = {0};   /* rtk residual file time */

static int azllevel = 0; /* rtk azimuth/elevation angle output level (0:off) */
static FILE *fp_azl = NULL; /* rtk azimuth/elevation angle file pointer */
static char file_azl[1024] =
    ""; /* rtk azimuth/elevation angle file original path */
static gtime_t time_azl = {0}; /* rtk azimuth/elevation angle file time */
...
extern int rtkopenamb(const char *file, int level) {
  gtime_t time = utc2gpst(timeget());
  char path[1024] = "";

  trace(3, "rtkopenamb: file=%s level=%d\n", file, level);

  if (level <= 0) return 0;

  reppath(file, path, time, "", "");

  if (!(fp_amb = fopen(path, "w"))) {
    trace(1, "rtkopenamb: file open error path=%s\n", path);
    return 0;
  }
  strcpy(file_amb, file);
  time_amb = time;
  amblevel = level;
  return 1;
}
extern int rtkopenres(const char *file, int level) {
  gtime_t time = utc2gpst(timeget());
  char path[1024] = "";

  trace(3, "rtkopenres: file=%s level=%d\n", file, level);

  if (level <= 0) return 0;

  reppath(file, path, time, "", "");

  if (!(fp_res = fopen(path, "w"))) {
    trace(1, "rtkopenres: file open error path=%s\n", path);
    return 0;
  }
  strcpy(file_res, file);
  time_res = time;
  reslevel = level;
  return 1;
}
extern int rtkopenazl(const char *file, int level) {
  gtime_t time = utc2gpst(timeget());
  char path[1024] = "";

  trace(3, "rtkopenazl: file=%s level=%d\n", file, level);

  if (level <= 0) return 0;

  reppath(file, path, time, "", "");

  if (!(fp_azl = fopen(path, "w"))) {
    trace(1, "rtkopenazl: file open error path=%s\n", path);
    return 0;
  }
  strcpy(file_azl, file);
  time_azl = time;
  azllevel = level;
  return 1;
}
...
/* close solution ammbiguity/residual/azl_ele file
 *-------------------------------------------------- close solution
 *ambiguity/residual/azl_ele file args   : none return : none
 *-----------------------------------------------------------------------------*/
extern void rtkcloseamb(void) {
  trace(3, "rtkcloseamb:\n");

  if (fp_amb) fclose(fp_amb);
  fp_amb = NULL;
  file_amb[0] = '\0';
  amblevel = 0;
}
extern void rtkcloseres(void) {
  trace(3, "rtkcloseres:\n");
...
extern void rtkcloseazl(void) {
  trace(3, "rtkcloseazl:\n");

  if (fp_azl) fclose(fp_azl);
  fp_azl = NULL;
  file_azl[0] = '\0';
  azllevel = 0;
}
...
extern int rtkoutstat(rtk_t *rtk, char *buff) {
  ...
  if (est) {
    ...
    p += sprintf(p, "$POS,%5d,%.3f,%3d,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f\n", week,
                 tow, rtk->sol.stat, rtk->x[0], rtk->x[1], rtk->x[2], xa[0],
                 xa[1], xa[2]);
  } else {
    p += sprintf(p, "$POS,%5d,%.3f,%3d,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f\n", week,
                 tow, rtk->sol.stat, rtk->sol.rr[0], rtk->sol.rr[1],
  ...
  if (est && rtk->opt.dynamics) {
    ...
    p += sprintf(p,
                 ...
                 "4f,%.5f,%.5f,%.5f\n",
                 week, tow, rtk->sol.stat, vel[0], vel[1], vel[2], acc[0],
                 acc[1], acc[2], vela[0], vela[1], vela[2], acca[0], acca[1],
                 acca[2]);
  } else {
    ecef2pos(rtk->sol.rr, pos);
    ...
    p += sprintf(p,
                 "$VELACC,%d,%.3f,%d,%.4f,%.4f,%.4f,%.5f,%.5f,%.5f,%.4f,%.4f,%."
                 "4f,%.5f,%.5f,%.5f\n",
                 week, tow, rtk->sol.stat, vel[0], vel[1], vel[2], 0.0, 0.0,
  ...
  if (est &&
      (rtk->opt.tropopt == TROPOPT_EST || rtk->opt.tropopt == TROPOPT_ESTG)) {
    for (i = 0; i < 2; i++) {
      j = IT(i, &rtk->opt);
      xa[0] = j < rtk->na ? rtk->xa[j] : 0.0;
...
/* write solution ambiguity to buffer
 * -------------------------------------------*/
extern int rtkoutamb(rtk_t *rtk, char *buff) {
  char *p = buff;

  if (rtk->sol.stat <= SOLQ_NONE) {
    return 0;
  }
  /* write ppp solution ambiguity to buffer */
  if (rtk->opt.mode >= PMODE_PPP_KINEMA) {
    return pppoutamb(rtk, buff);
  }
...
static void swapsolamb(void) {
  gtime_t time = utc2gpst(timeget());
  char path[1024] = "";

  if ((int)(time2gpst(time, NULL) / INT_SWAP_AMB) ==
      (int)(time2gpst(time_amb, NULL) / INT_SWAP_AMB)) {
    return;
  }
  time_amb = time;

  if (!reppath(file_amb, path, time, "", "")) {
    return;
  }
  if (fp_amb) fclose(fp_amb);

  if (!(fp_amb = fopen(path, "w"))) {
    trace(2, "swapsolamb: file open error path=%s\n", path);
    return;
  }
  trace(3, "swapsolamb: path=%s\n", path);
}
static void swapsolres(void) {
  gtime_t time = utc2gpst(timeget());
  char path[1024] = "";

  if ((int)(time2gpst(time, NULL) / INT_SWAP_RES) ==
      (int)(time2gpst(time_res, NULL) / INT_SWAP_RES)) {
    return;
  }
  time_res = time;

  if (!reppath(file_res, path, time, "", "")) {
    return;
  }
  if (fp_res) fclose(fp_res);

  if (!(fp_res = fopen(path, "w"))) {
    trace(2, "swapsolres: file open error path=%s\n", path);
    return;
  }
  trace(3, "swapsolres: path=%s\n", path);
}
static void swapsolazl(void) {
  gtime_t time = utc2gpst(timeget());
  char path[1024] = "";

  if ((int)(time2gpst(time, NULL) / INT_SWAP_AZL) ==
      (int)(time2gpst(time_azl, NULL) / INT_SWAP_AZL)) {
    return;
  }
  time_azl = time;

  if (!reppath(file_azl, path, time, "", "")) {
    return;
  }
  if (fp_azl) fclose(fp_azl);

  if (!(fp_azl = fopen(path, "w"))) {
    trace(2, "swapsolazl: file open error path=%s\n", path);
    return;
  }
  trace(3, "swapsolazl: path=%s\n", path);
}
...
/* output solution ambiguity/residual/azimuth_elevation angle for PPP (GPS only)
 * -------*/
static void outsolamb(rtk_t *rtk) {
  char buff[MAXSOLMSG + 1] = "";
  int n;

  if (amblevel <= 0 || !fp_amb || !rtk->sol.stat) return;

  trace(3, "outsolamb:\n");

  /* swap solution ambiguity file */
  swapsolamb();

  /* write solution ambiguity */
  n = rtkoutamb(rtk, buff);
  buff[n] = '\0';

  fputs(buff, fp_amb);
}
static void outsolres(rtk_t *rtk) {
  ssat_t *ssat;
  char str[32] = "";
  int i, j, nfreq, nf = NF(&rtk->opt);

  if (reslevel <= 0 || !fp_res || !rtk->sol.stat) return;

  trace(3, "outsolres:\n");

  /* swap solution residual file */
  swapsolres();

  if (rtk->sol.stat == SOLQ_NONE || reslevel <= 0) return;
  ...
  for (i = 0; i < NSATGPS; i++) {
    ssat = rtk->ssat + i;
    for (j = 0; j < nfreq; j++) {
      fprintf(fp_res, "%8.4f %8.4f %3d  ", ssat->resp[j], ssat->resc[j],
              ssat->vsat[j]);
    }
  }
  fprintf(fp_res, "\n");
}
static void outsolazl(rtk_t *rtk) {
  ssat_t *ssat;
  char str[32] = "";
  int i, j, nfreq, nf = NF(&rtk->opt);

  if (azllevel <= 0 || !fp_azl || !rtk->sol.stat) return;

  trace(3, "outsolazl:\n");

  /* swap solution azimuth/elevation angle file */
  swapsolazl();

  if (rtk->sol.stat == SOLQ_NONE || azllevel <= 0) return;

  nfreq = rtk->opt.mode >= PMODE_DGPS ? nf : 1;

  time2str(rtk->sol.time, str, 2);
  fprintf(fp_azl, "%s ", str);
  /* write azimuth/elevation angle */
  for (i = 0; i < NSATGPS; i++) {
    ssat = rtk->ssat + i;
    for (j = 0; j < nfreq; j++) {
      fprintf(fp_azl, "%8.3f %8.3f %3d  ", ssat->azel[0] * R2D,
              ssat->azel[1] * R2D, ssat->vsat[j]);
    }
  }
  fprintf(fp_azl, "\n");
}
...
static double varerr(int sat, int sys, double el, double bl, double dt, int f,
  ...
  if (f >= nf && opt->exterr.ena[0]) { /* code */
    ...
    fact *=
        sys == SYS_GLO ? EFACT_GLO : (sys == SYS_SBS ? EFACT_SBS : EFACT_GPS);
    a = fact * opt->err[1];
    b = fact * opt->err[2];
  }
  return 2.0 * (opt->ionoopt == IONOOPT_IFLC ? 3.0 : 1.0) *
             (a * a + b * b / sinel / sinel + c * c) +
         d * d;
}
/* baseline length -----------------------------------------------------------*/
...
static void detslp_gf_L1L2(rtk_t *rtk, const obsd_t *obs, int i, int j,
  ...
  if (rtk->opt.nf <= 1 ||
      (g1 = gfobs_L1L2(obs, i, j, nav->lam[sat - 1])) == 0.0)
    return;
...
static void detslp_gf_L1L5(rtk_t *rtk, const obsd_t *obs, int i, int j,
  ...
  if (rtk->opt.nf <= 2 ||
      (g1 = gfobs_L1L5(obs, i, j, nav->lam[sat - 1])) == 0.0)
    return;
...
static int zdres(int base, const obsd_t *obs, int n, const double *rs,
  ...
  for (i = 0; i < n; i++) {
    trace(4, "sat=%2d %13.3f %13.3f %13.3f %13.10f %6.1f %5.1f\n", obs[i].sat,
          rs[i * 6], rs[1 + i * 6], rs[2 + i * 6], dts[i * 2],
...
static int validobs(int i, int j, int f, int nf, double *y) {
  ...
  return y[f + i * nf * 2] != 0.0 && y[f + j * nf * 2] != 0.0 &&
         (f < nf ||
          (y[f - nf + i * nf * 2] != 0.0 && y[f - nf + j * nf * 2] != 0.0));
}
...
static int ddres(rtk_t *rtk, const nav_t *nav, double dt, const double *x,
                 ...
                 double *H, double *R, int *vflg, const double *rs) {
  prcopt_t *opt = &rtk->opt;
  double bl, dr[3], posu[3], posr[3], didxi = 0.0, didxj = 0.0, *im;
  double *tropr, *tropu, *dtdxr, *dtdxu, *Ri, *Rj, lami, lamj, fi, fj, df,
      *Hi = NULL;
  int i, j, k, m, f, ff, nv = 0, nb[NFREQ * 4 * 2 + 2] = {0}, b = 0, sysi, sysj,
                         nf = NF(opt);
  ...
  for (i = 0; i < ns; i++) {
    if (opt->ionoopt >= IONOOPT_EST) {
      im[i] =
          (ionmapf(posu, azel + iu[i] * 2) + ionmapf(posr, azel + ir[i] * 2)) /
          2.0;
    }
    if (opt->tropopt >= TROPOPT_EST) {
      tropu[i] = prectrop(rtk->sol.time, posu, 0, azel + iu[i] * 2, opt, x,
                          dtdxu + i * 3);
      tropr[i] = prectrop(rtk->sol.time, posr, 1, azel + ir[i] * 2, opt, x,
                          dtdxr + i * 3);
    }
  }
      ...
      for (j = 0; j < ns; j++) {
        ...
        if (f < nf) {
          if (opt->ionoopt != IONOOPT_IFLC) {
            // double Ai = x[IB(sat[i], f, opt)];
            // double Aj = x[IB(sat[j], f, opt)];
            // printf("%d,%5.3f\n", sat[j], Aj);
            v[nv] -=
                lami * x[IB(sat[i], f, opt)] - lamj * x[IB(sat[j], f, opt)];
            if (H) {
        ...
        if (rtk->opt.glomodear == 2 && sysi == SYS_GLO && sysj == SYS_GLO &&
            ff < NFREQGLO) {
          df =
              (CLIGHT / lami - CLIGHT / lamj) / 1E6; /* freq-difference (MHz) */
          v[nv] -= df * x[IL(ff, opt)];
        ...
        trace(4, "sat=%3d-%3d %s%d v=%13.3f R=%8.6f %8.6f\n", sat[i], sat[j],
              f < nf ? "L" : "P", f % nf + 1, v[nv], Ri[nv], Rj[nv]);

        vflg[nv++] =
            (sat[i] << 16) | (sat[j] << 8) | ((f < nf ? 0 : 1) << 4) | (f % nf);
        nb[b]++;
      }
...
static int resamb_LAMBDA(rtk_t *rtk, double *bias, double *xa) {
  ...
  for (i = 0; i < nb; i++)
    for (j = 0; j < nb; j++) Qb[i + j * nb] = Qy[na + i + (na + j) * ny];
  for (i = 0; i < na; i++)
    for (j = 0; j < nb; j++) Qab[i + j * na] = Qy[i + (na + j) * ny];

  trace(6, "rtk->P=\n");
  tracemat(6, rtk->P, nx, nx, 10, 3);
  
  trace(6, "N(0)=\n");
  tracemat(6, y + na, 1, nb, 10, 3);

  trace(6, "Qb=\n");
  tracemat(6, Qb, nb, nb, 10, 3);
...
static int relpos(rtk_t *rtk, const obsd_t *obs, int nu, int nr,
  ...
  for (i = 0; i < MAXSAT; i++) {
    ...
    for (j = 0; j < NFREQ; j++) rtk->ssat[i].snr[j] = 0;
  }
  ...
  if (!zdres(1, obs + nu, nr, rs + nu * 6, dts + nu * 2, svh + nu, nav, rtk->rb,
             opt, 1, y + nu * nf * 2, e + nu * 3, azel + nu * 2)) {
  ...
  niter =
      opt->niter + (opt->mode == PMODE_MOVEB && opt->baseline[0] > 0.0 ? 2 : 0);
  ...
  for (i = 0; i < niter; i++) {
    ...
    if ((nv = ddres(rtk, nav, dt, xp, Pp, sat, y, e, azel, iu, ir, ns, v, H, R,
                    vflg, rs)) < 1) {
  ...
  if (stat != SOLQ_NONE &&
    ...
    nv = ddres(rtk, nav, dt, xp, Pp, sat, y, e, azel, iu, ir, ns, v, NULL, R,
               vflg, rs);
 ...
 *            rtk->tt        O   time difference between current and previous
 *(s) rtk->x[]       IO  float states pre-filter and post-filter rtk->P[] IO
 *float covariance pre-filter and post-filter rtk->xa[]      O   fixed states
 *after AR rtk->Pa[]      O   fixed covariance after AR rtk->ssat[s]   IO
 *sat(s+1) status .sys       O   system (SYS_???) .az   [r]  O   azimuth angle
 *(rad) (r=0:rover,1:base) .el   [r]  O   elevation angle (rad)
 *(r=0:rover,1:base) .vs   [r]  O   data valid single     (r=0:rover,1:base)
