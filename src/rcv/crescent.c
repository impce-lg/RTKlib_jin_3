...
static const char rcsid[] =
    "$Id: crescent.c,v 1.2 2008/07/14 00:05:05 TTAKA Exp $";
...
static int chksum(const unsigned char *buff, int len) {
  ...
  trace(4, "checksum=%02X%02X %02X%02X:%02X%02X\n", sum >> 8, sum & 0xFF,
        buff[len - 3], buff[len - 4], buff[len - 2], buff[len - 1]);
...
static int decode_cresraw(raw_t *raw) {
  ...
  for (i = n = 0, p += 12; i < 12 && n < MAXOBS; i++, p += 24) {
    ...
    if (raw->time.time != 0) {
      lli =
          (int)((unsigned char)sc - (unsigned char)raw->lockt[sat - 1][0]) > 0;
    }
...
static int decode_cresraw2(raw_t *raw) {
  gtime_t time;
  double tow, tows, toff = 0.0, cp[2] = {0}, pr1, pr[2] = {0}, dop[2] = {0},
                    snr[2] = {0};
  ...
  for (i = 0, p += 16; i < 15 && n < MAXOBS; i++) {
    ...
    if (cp[0] - pr[0] / lam_carr[0] < -4096.0)
      cp[0] += 8192.0;
    else if (cp[0] - pr[0] / lam_carr[0] > 4096.0)
      cp[0] -= 8192.0;
    ...
    if (i < 12) {
      ...
      sc = (unsigned int)(word1 >> 24);
      if (raw->time.time == 0) {
        lli[1] = (int)((unsigned char)sc -
                       (unsigned char)raw->lockt[sat - 1][1]) > 0;
      } else {
        lli[1] = 0;
      }
      ...
      if (pr[1] != 0.0) {
        ...
        cp[1] +=
            ((word2 & 0xFE000000) + ((word3 & 0xFFFF0000) >> 7)) / 524288.0;
        if (cp[1] - pr[1] / lam_carr[1] < -4096.0)
          cp[1] += 8192.0;
        else if (cp[1] - pr[1] / lam_carr[1] > 4096.0)
          cp[1] -= 8192.0;
      } else
        cp[1] = 0.0;
    }
...
static int decode_creswaas(raw_t *raw) {
  ...
  for (i = k = 0; i < 8 && k < 29; i++) {
    word = U4(p + 8 + i * 4);
    for (j = 0; j < 4 && k < 29; j++)
      raw->sbsmsg.msg[k++] = (unsigned char)(word >> (3 - j) * 8);
  }
...
static int decode_cresgloraw(raw_t *raw) {
  ...
  for (i = 0, p += 16; i < 12 && n < MAXOBS; i++) {
    ...
    if (raw->time.time == 0) {
      lli[1] =
          (int)((unsigned char)sc - (unsigned char)raw->lockt[sat - 1][1]) > 0;
    } else {
      lli[1] = 0;
    }
    lli[1] |= ((word1 >> 12) & 7) ? 2 : 0;
