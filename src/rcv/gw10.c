...
/* bcd to number -------------------------------------------------------------*/
static int bcd2num(unsigned char bcd) { return (bcd >> 4) * 10 + (bcd & 0xF); }
/* decode raw obs data -------------------------------------------------------*/
...
static int decode_gw10(raw_t *raw) {
  ...
  switch (type) {
    case ID_GW10RAW:
      return decode_gw10raw(raw);
    case ID_GW10GPS:
      return decode_gw10gps(raw);
    case ID_GW10SBS:
      return decode_gw10sbs(raw);
    case ID_GW10REPH:
      return decode_gw10reph(raw);
    case ID_GW10SOL:
      return decode_gw10sol(raw);
  }
  return 0;
}
