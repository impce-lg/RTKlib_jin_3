...
static int decode_xf5raw(raw_t *raw) {
  ...
  for (i = 0, p += 27; (i < nsat) && (n < MAXOBS); i++, p += 30) {
    ...
    sys = (U1(p) == 1)
              ? SYS_GLO
              : ((U1(p) == 2) ? SYS_GPS : ((U1(p) == 4) ? SYS_SBS : SYS_NONE));
    prn = U1(p + 1);
    ...
    /* check range error */
    if (L1 < -1E10 || L1 > 1E10 || P1 < -1E10 || P1 > 1E10 || D1 < -1E5 ||
        D1 > 1E5) {
      trace(
          2,
          "nvs xf5raw obs range error: sat=%2d L1=%12.5e P1=%12.5e D1=%12.5e\n",
          sat, L1, P1, D1);
      continue;
    }
    ...
    raw->obs.data[n].P[0] = (P1 - dTowFrac) * CLIGHT * 0.001 -
                            toff * CLIGHT; /* in ms, needs to be converted */
    raw->obs.data[n].D[0] = (float)D1;

    /* set LLI if meas flag 4 (carrier phase present) off -> on */
    flag = U1(p + 28);
    raw->obs.data[n].LLI[0] =
        (flag & 0x08) && !(raw->halfc[sat - 1][0] & 0x08) ? 1 : 0;
  ...
  for (iIdx = 0; iIdx < uiDataBlocks; iIdx++) {
    ...
    switch (uiDataType) {
      ...
      case 4: /* SBAS */
        ...
        for (iIdx = 0, iBlkStartIdx += 7; iIdx < 10;
             iIdx++, iBlkStartIdx += 4) {
