 ...
 *         GLONASS/GPS Receiver (Raw Measurement F/W), ver 1.4.26, April 26,
 *2012 [4] Skytraq, Application Note AN0030 Binary Message of Raw Measurement
...
static int decode_stqgps(raw_t *raw) {
  ...
  prn = U1(p + 1);
  if (!(sat = satno(MINPRNQZS <= prn && prn <= MAXPRNQZS ? SYS_QZS : SYS_GPS,
                    prn))) {
...
extern int gen_stq(const char *msg, unsigned char *buff) {
  ...
  *q++ = STQSYNC2;
  if (!strcmp(args[0], "RESTART")) {
    ...
    *q++ = ID_CFGSERI;
    for (i = 1; i < 4; i++)
      *q++ = narg > i + 1 ? (unsigned char)atoi(args[i]) : 0;
