...
EXPORT int init_rt17(raw_t *Raw) {
  ...
  if (!(MessageBuffer =
            (unsigned char *)calloc(MBUFF_LENGTH, sizeof(unsigned char)))) {
  ...
  if (!(PacketBuffer =
            (unsigned char *)calloc(PBUFF_LENGTH, sizeof(unsigned char)))) {
...
EXPORT int input_rt17(raw_t *Raw, unsigned char Data) {
  ...
  */
  ...
  if (Raw->outtype)
    sprintf(Raw->msgtype, "RT17 0x%02X (%4d)", PacketBuffer[2],
            rt17->PacketLength);
...
static int DecodeType17(raw_t *Raw, unsigned int rif) {
  ...
  for (i = n = 0; (i < nsat) && (i < MAXOBS); i++) {
    ...
    obs->code[0] = (obs->P[0] == 0.0) ? CODE_NONE
                                      : (Flags2 & M_BIT0) ? CODE_L1P : CODE_L1C;
    obs->code[1] = (obs->P[1] == 0.0)
                       ? CODE_NONE
                       : (Flags2 & M_BIT2)
                             ? CODE_L2W
                             : (Flags2 & M_BIT1) ? CODE_L2P : CODE_L2C;
