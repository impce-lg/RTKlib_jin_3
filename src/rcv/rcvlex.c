...
static int decode_lexraw(raw_t *raw) {
  ...
  for (i = 0; i < 17 && p + 38 <= raw->buff + raw->len - 4; i++) {
    ...
    node = U1(p);
    p += 1; /* cpu board node id */
    prn = U1(p);
    p += 1; /* prn code number (193-197) */
    stat = U1(p);
    p += 1; /* tracking status (b0-b3) */
    ...
    p += 8; /* pseudorange (m) */
    dop = R8(p);
    p += 8; /* doppler frequency (Hz) */
    adr = R8(p);
    p += 8; /* accumlated delta range (cycle) */
    acc = U2(p);
    p += 2; /* accurated time of delta range (ms) */
    ttt = U4(p);
    p += 4; /* tracking time (ms) */
...
static int decode_lexmsg(raw_t *raw) {
  ...
  p += 1; /* channel number */
  sig = U1(p);
  p += 1; /* signal type (0x1A:lex short) */
  prn = U1(p);
  p += 1; /* prn number */
  err = U2(p);
  p += 2; /* err status */
  ...
  i = 0;
  preamb = getbitu(p, i, 32);
  i += 32; /* preamble */
  prnmsg = getbitu(p, i, 8);
  i += 8; /* prn number */
  type = getbitu(p, i, 8);
  i += 8; /* lex message type */
  alert = getbitu(p, i, 1);
  i += 1; /* alert flag */
  ...
  for (j = 0; j < 212; j++) {
    raw->lexmsg.msg[j] = (unsigned char)getbitu(p, i, 8);
    i += 8;
  }
...
static int decode_lexr(raw_t *raw) {
  ...
  switch (type) {
    case ID_LEXRAW:
      return decode_lexraw(raw);
    case ID_LEXMSG:
      return decode_lexmsg(raw);
  }
  return 0;
}
/* sync code -----------------------------------------------------------------*/
...
extern int gen_lexr(const char *msg, unsigned char *buff) {
  ...
  if (!strcmp(args[0], "CMD-RST")) {
    ...
    if (narg < 2) {
      ...
      if (narg > 2)
        sscanf(args[2], "%lf:%lf:%lf", epoch + 3, epoch + 4, epoch + 5);
    }
    ...
    for (i = 0; i < 3; i++)
      if (i + 1 < narg) pos[i] = atof(args[i + 1]) * (i < 2 ? D2R : 1.0);
    *q++ = 0;
