 ...
 *(2.4.0_p9) 2011/05/27 1.7  add almanac decoding add -EPHALL option fix problem
 *with ARM compiler 2013/02/23 1.8  fix memory access violation problem on arm
...
static const char rcsid[] =
    "$Id: ublox.c,v 1.2 2008/07/14 00:05:05 TTAKA Exp $";
...
static int decode_rxmrawx(raw_t *raw) {
  ...
  int i, j, sys, prn, sat, n = 0, nsat, week, tstat, lockt, slip, halfv, halfc,
                           fcn, cpstd;
  int std_slip = 0;
  char *q;
  unsigned char *p = raw->buff + 6;
...
static int decode_trkd5(raw_t *raw) {
  ...
  for (i = 0, p = raw->buff + off; p - raw->buff < raw->len - 2;
       i++, p += len) {
 ...
 *            "CFG-TP    interval length status time_ref res adelay rdelay
 *udelay" "CFG-NAV2  ..." "CFG-DAT   maja flat dx dy dz rotx roty rotz scale"
 *            "CFG-INF   protocolid res0 res1 res2 mask0 mask1 mask2 ... mask5"
 *            "CFG-RST   navbbr reset res"
 *            "CFG-RXM   gpsmode lpmode"
 *            "CFG-ANT   flags pins"
 *            "CFG-FXN   flags treacq tacq treacqoff tacqoff ton toff res
 *basetow" "CFG-SBAS  mode usage maxsbas res scanmode" "CFG-LIC   key0 key1 key2
 *key3 key4 key5" "CFG-TM    intid rate flags" "CFG-TM2   ch res0 res1 rate
 *flags" "CFG-TMODE tmode posx posy posz posvar svinmindur svinvarlimit"
 *            "CFG-EKF   ..."
 *            "CFG-GNSS  ..."
 *            "CFG-ITFM  conf conf2"
...
extern int gen_ubx(const char *msg, unsigned char *buff) {
  const char *cmd[] = {
      "PRT",  "USB",       "MSG",    "NMEA",   "RATE",  "CFG",    "TP",
      "NAV2", "DAT",       "INF",    "RST",    "RXM",   "ANT",    "FXN",
      "SBAS", "LIC",       "TM",     "TM2",    "TMODE", "EKF",    "GNSS",
      "ITFM", "LOGFILTER", "NAV5",   "NAVX5",  "ODO",   "PM2",    "PWR",
      "RINV", "SMGR",      "TMODE2", "TMODE3", "TPS",   "TXSLOT", ""};
  const unsigned char id[] = {
      0x00, 0x1B, 0x01, 0x17, 0x08, 0x09, 0x07, 0x1A, 0x06, 0x02, 0x04, 0x11,
      0x13, 0x0E, 0x16, 0x80, 0x10, 0x19, 0x1D, 0x12, 0x3E, 0x39, 0x47, 0x24,
      0x23, 0x1E, 0x3B, 0x57, 0x34, 0x62, 0x36, 0x71, 0x31, 0x53};
  const int prm[][32] = {
