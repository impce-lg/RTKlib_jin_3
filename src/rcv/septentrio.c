 ...
 *
 * note: - QZSS and Compass/Beidou is deactivated. The code is not tested. Use
 *-DTESTING to activate.
 *
...
#define ID_MEASEPOCHEXTRA \
  4000 /* SBF message id: range measurememts extra info */
#define ID_MEASEPOCH_END \
...
#define ID_GEORAWL5 4021  /* SBF message id: SBAS raw navigation page or frame \
                           */
#define ID_GALRAWFNAV \
  4022 /* SBF message id: Galileo raw navigation page or frame */
#define ID_GALRAWINAV \
  4023 /* SBF message id: Galileo raw navigation page or frame */
#define ID_GALRAWCNAV \
  4024 /* SBF message id: Galileo raw navigation page or frame */
#define ID_GLORAWCA \
  4026 /* SBF message id: GLONASS raw navigation page or frame */
#define ID_CMPRAW \
  4047 /* SBF message id: Compass raw navigation page or frame */
...
#define ID_IRNSSRAW \
  4093 /* SBF message id: IRNSS raw navigation page or frame */
...
#define ID_GEOIONODELAY 5933 /* SBF message id:  Inospheric delay correction \
                              */
#define ID_GEOCLOCKEPHCOVMATRIX \
...
#define ID_PVTCART                                                        \
  4006 /* SBF message id: Rx Position Velocity and Time data in Cartesian \
          coordinates in m */
#define ID_PVTGEOD                                                       \
...
#define ID_PVTSATCART \
  4008 /* SBF message id: Satellite Position Velocity and Time data */

#define ID_ENDOFPVT 5921 /* SBF message id: End of any PVT block */

#define ID_RXTIME 5914 /* SBF message id: Receiver time data */

#define ID_DIFFCORRIN \
  5919 /* SBF message id: incoming RTCM2 or RTCM3 or CMR message */
...
#define ID_BBSMPS \
  4040 /* SBF message id: series of successive Rx baseband samples */
...
static signed int I4(unsigned char *p) {
  signed int u;
  memcpy(&u, p, 4);
  return u;
}
static short I2(unsigned char *p) {
  short i;
  memcpy(&i, p, 2);
  return i;
}
...
static int decode_measepoch(raw_t *raw) {
  ...
  */
  ...
  if ((CommonFlags & 0x80) == 0x80)
    return 0; /* data is ccrambled and not valid */
  ...
  for (i = 0; i < nsat && i < MAXOBS; i++) {
    ...
    if ((signType1 == 1) || (signType1 == 2)) {
      ...
      SNR_DBHZ = (((double)U1(p + 15)) * 0.25) + 10;
    ...
    adr = (SB1_Code / SB1_WaveLength) +
          (I1(p + 14) * 65536.0 + U2(p + 12)) * 0.001;
    ...
    if (freqType1 == FREQ1)
      h = 0;
    else if (freqType1 == FREQ2)
      h = 1;
    else if (freqType1 == FREQ5)
      h = 2;
    else if (freqType1 == FREQ6)
      h = 3;
    else if (freqType1 == FREQ7)
      h = 4;
    else if (freqType1 == FREQ8)
      h = 5;
    else
      h = 0;
#endif
    ...
    if (h <= NFREQ + NEXOBS) {
      ...
      if ((ObsInfo & 0x4) == 0x4)
        raw->obs.data[n].LLI[h] |= 0x2; /* half-cycle ambiguity */
      if (LockTime != 65535) {
        LockTime = LockTime > 254
                       ? 254
                       : LockTime; /* limit locktime to sizeof(unsigned char) */
    ...
    for (ii = 0; ii < (int)SB2Num; ii++) {
      ...
      if ((signType2 == 1) || (signType2 == 2)) {
        ...
        SNR2_DBHZ = (((double)U1(p + 2)) * 0.25) + 10;

      offsetMSB = U1(p + 3);
      CodeOffsetMSB = ((offsetMSB & 0x04) == 0x04)
                          ? offsetMSB | ~((int32_t)0x03)
                          : offsetMSB & 0x03; /* bit[0-2] */
      DopplerOffsetMSB = ((offsetMSB & 0x80) == 0x80)
                             ? (offsetMSB >> 3) | ~((int32_t)0x1f)
                             : (offsetMSB >> 3) & 0x1f; /* bit[3-7] */
      ...
      CarrierLSB = U2(p + 8);
      Ltype2 = (PRtype2 / CLIGHT * freqType2) +
      ...
      if ((DopplerOffsetMSB == -16) && (DopplerOffsetLSB == 0))
        dopplerType2 = 0;
      else
      ...
      if (freqType2 == FREQ1)
        h = 0;
      else if (freqType2 == FREQ2)
        h = 1;
      else if (freqType2 == FREQ5)
        h = 2;
      else if (freqType2 == FREQ6)
        h = 3;
      else if (freqType2 == FREQ7)
        h = 4;
      else if (freqType2 == FREQ8)
        h = 5;
      else
        h = 0;
#endif
      pri = getcodepri(sys, getSignalCode(signType2),
                       raw->opt); /* get signal priority */
      /* store signal info */
      if ((h <= NFREQ + NEXOBS) &&
        ...
        if ((ObsInfo2 & 0x4) == 0x4)
          raw->obs.data[n].LLI[h] |= 0x2; /* half-cycle ambiguity */
        if (LockTime2 != 255) {
          if (locktime[sat][signType2] > LockTime2)
            raw->obs.data[n].LLI[h] |= 0x1;
  ...
  if (id == 4) {
    if (sys == SYS_GPS) {
      decode_frame(raw->subfrm[sat - 1] + 90, NULL, raw->nav.alm,
                   raw->nav.ion_gps, raw->nav.utc_gps, &raw->nav.leaps);
      adj_utcweek(raw->time, raw->nav.utc_gps);
    } else if (sys == SYS_QZS) {
      decode_frame(raw->subfrm[sat - 1] + 90, NULL, raw->nav.alm,
                   raw->nav.ion_qzs, raw->nav.utc_qzs, &raw->nav.leaps);
...
static int decode_galrawinav(raw_t *raw) {
  ...
  /* test crc */
  for (i = 0, j = 4; i < 49; i++, j += 4)
    setbitu(crc_buff, j, 4, getbitu(buff, i * 4, 4));
  ...
  if ((U4(p + 14) & 0x80) != 0x80) /* E5b-I */
  {
    ...
    for (i = 0, j = 2; i < 16; i++, j += 8)
      raw->subfrm[sat - 1][pos++] = getbitu(buff, j, 8);
  } else { /* E1-B */
    int pos, i;
...
static int decode_cmpraw(raw_t *raw) {
  ...
  for (i = 0; i < 10; i++)
    words[i] = U4(p + 12 + i * 4) & 0x3FFFFFFF; /* 30 bits */
...
static int decode_sbsfast(raw_t *raw) {
  ...
  for (i = 0; i < sbCount; i++) {
    if ((j = 13 * ((type == 0 ? 2 : type) - 2) + i) >= raw->nav.sbssat.nsat)
      break;
    ...
    if (t0_old.time == 0 || dt <= 0.0 || 18.0 < dt ||
        raw->nav.sbssat.sat[j].fcorr.ai == 0) {
      raw->nav.sbssat.sat[j].fcorr.rrc = 0.0;
      raw->nav.sbssat.sat[j].fcorr.dt = 0.0;
    } else {
      raw->nav.sbssat.sat[j].fcorr.rrc =
          (raw->nav.sbssat.sat[j].fcorr.prc - prc_old) / dt;
...
static int decode_sbsigpmask(raw_t *raw) /* TODO: verify this function */
{
  ...
  for (n = 0; n < raw->nav.sbsion[band].nigp; n++) {
    ...
    for (j = 0; j < m; j++) {
      ...
      raw->nav.sbsion[band].igp[n].lat =
          band <= 8 ? b[j].y[i - b[j].bits] : b[j].x;
      raw->nav.sbsion[band].igp[n++].lon =
          band <= 8 ? b[j].x : b[j].y[i - b[j].bits];
      break;
    }
  }
...
static int decode_sbslongcorrh(raw_t *raw) {
  ...
  for (i = 0; i < count; i++) {
    ...
    if (U1(p + 14 + i * sbLength) == 1) {
      raw->nav.sbssat.sat[no - 1].lcorr.dvel[i] =
          R4(p + 14 + i * sbLength + 16);
      raw->nav.sbssat.sat[no - 1].lcorr.dvel[i] =
          R4(p + 14 + i * sbLength + 20);
      raw->nav.sbssat.sat[no - 1].lcorr.dvel[i] =
          R4(p + 14 + i * sbLength + 24);

      raw->nav.sbssat.sat[no - 1].lcorr.daf1 = R4(p + 14 + i * sbLength + 32);
    } else {
      raw->nav.sbssat.sat[no - 1].lcorr.dvel[0] =
          raw->nav.sbssat.sat[no - 1].lcorr.dvel[1] =
...
static int decode_sbf(raw_t *raw) {
  ...
  switch (type) {
    ...
    case ID_QZSSL2C:
    case ID_QZSSL5:
      return decode_rawnav(raw, SYS_QZS);
      l case ID_QZSS_NAV : return decode_qzssnav(raw);
#endif
