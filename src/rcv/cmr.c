...
|
| For testing purposes one would normally use the Trimble GPS Configurator
utility | to configure the test receiver, but that utility does not understand
all receiver | features such as CMR 5Hz and 10Hz output. The Trimble WINPAN
utility was used | to configure CMR 5Hz and 10Hz output.
|
| For the purposes of this explaination the following terms mean the following
things:
|
...
|
| We temporarilly store them internally as "unreferenced" observables as
follows:
|
...
|
| 1. CBT (Actually just once in the messsage header and it's modulo 4000 instead
of 240000.) | 2. CL1 (But it's a delta against the prior CMR type 3 BL1 for this
satellte.)
|
...
| Public functions appear as a set first (there are only three of them),
followed | by private functions as a set. Because of this, forward definitions
are required | for the private functions. Please keep that in mind when making
changes to this | source file.
|
| References:
|
...
/* Utility macros: */
#define SNRATIO(snr) \
  (unsigned char)((snr <= 0.0) || (snr >= 255.5)) ? (0.0) : ((snr * 4.0) + 0.5)
...
typedef struct { /* CMR information struct type */
  unsigned char
      *Buffer; /* Buffer for building full CMR+ message from little parts */
  unsigned char *MessageBuffer; /* Message buffer */
  obsr_t *RoverObservables;     /* Rover observables table */
  rtksvr_t *Svr;      /* Pointer to RTK server structure (when running in that
...
static const ant_t AntennasTable[] = {
    ...
    {26,
     "LEISRX99+GP     NONE"}, /* Leica SR299/SR399 External w/Ground Plane */
    ...
    {34,
     "TRM22020.00+GP  TCWD"}, /* Trimble Compact L1/L2 w/Ground Plane+Radome */
    ...
    {40,
     "TRM33429.00+GP  NONE"}, /* Trimble Micro-centered L1/L2 w/Ground Plane */
    ...
    {47,
     "TRM36569.00+GP  NONE"}, /* Trimble Rugged Micro-centered w/13Inch GP */
    {48, "GeoExplorer 3"},    /* Trimble GeoExplorer 3 */
    {49, "TRM33429.20+GP  NONE"}, /* Trimble Micro-centered L1/L2 Permanent */
    {50, "TRM33429.20+GP  TCWD"}, /* Trimble Micro-centered L1/L2 Permanent
                                     w/Radome */
    ...
    {58, "JPSLEGANT_E     NONE"}, /* Javad Positioning Systems JPS Legant w/Flat
                                     Groundplane */
    {59, "JPSREGANT_SD_E  NONE"}, /* Javad Positioning Systems JPS Regant
                                     w/Single Depth Choke, Ext */
    {60, "JPSREGANT_DD_E  NONE"}, /* Javad Positioning Systems JPS Regant w/Dual
                                     Depth Choke, Ext */
    ...
    {91, "SPPGEOTRACER2000NONE"}, /* Spectra Precision SPP Geotracer 2000/2100
                                     L1 Internal */
    ...
    {158, "TRM33429.20+GP  UNAV"}, /* Trimble Micro-centered L1/L2 Perm. w/UNAV
                                      Dome */
    ...
    {183,
     "Recon GPS CF Card Internal"},  /* Trimble Recon GPS CF Card Internal */
    ...
    {207,
     "TRM41249USCG    SCIT"}, /* Trimble Zephyr Geodetic w/USCG SCIT Dome */
    ...
    {258,
     "JNSCR_C146-22-1 NONE"}, /* Javad Navigation Systems JNSCR_C146-22-1 */
    ...
    {276,
     "TRM57971.00     SCIT"}, /* Trimble Zephyr Geodetic 2 RoHS w/SCIT Dome */
    {277,
     "TRM57971.00     TZGD"}, /* Trimble Zephyr Geodetic 2 RoHS w/TZGD Dome */
    ...
    {350, "AOAD/M_B        OSOD"}, /* Allen Osborne Associates AOAD Model B
                                      w/OSOD Dome */
    {351, "AOAD/M_T        OSOD"}, /* Allen Osborne Associates AOAD Model T
                                      w/OSOD Dome */
    ...
    {363, "JNSCR_C146-22-1 OSOD"}, /* Javad Navigation Systems JNSCR_C146-22-1
                                      w/OSOD DOme */
    ...
    {377,
     "JNSCHOKERING_DM NONE"}, /* Javad Navigation Systems JNSCHOKERING_DM */
    {378, "JNSCHOKERING_DM OSOD"}, /* Javad Navigation Systems JNSCHOKERING_DM
                                      w/OSOD DOme */
...
| A handful of receivers also have an "OldRxId" entry which provides an
additional | number with the same name.
|
...
| ### EXTENDS THIS OTHERWISE SOMEONE DROPPED THE BALL...                     ###
*/
static const rcv_t ReceiversTable[] = {{1, "TRIMBLE 4000A"},
                                       {2, "TRIMBLE 4000ST"},
                                       {3, "TRIMBLE 4000SE"},
                                       {4, "TRIMBLE 4000SE"},
                                       {5, "TRIMBLE 4000SI"},
...
static const unsigned char SnrTable[][2] = {
    {0, 0},   {30, 4},  {32, 8},  {34, 12}, {36, 16}, {38, 20},
    {40, 24}, {42, 28}, {44, 32}, {46, 36}, {48, 40}, {50, 44},
...
static int ReferenceCmrObs(raw_t *Raw, gtime_t Time, unsigned char Type,
                           double P0, obsbd_t *Obs);
static gtime_t ReferenceCmrTime(gtime_t CmrTime, gtime_t RoverTime,
                                double WindowSize);
...
static size_t TrimCopy(char *Destination, size_t DestinationLength,
                       char *Source, size_t SourceLength);
static unsigned int ubitn(const unsigned char *Address, int BitPosition,
                          int BitLength);
...
EXPORT void free_cmr(raw_t *Raw) {
  ...
  if (Raw->format != STRFMT_CMR) return;
...
EXPORT int init_cmr(raw_t *Raw) {
  ...
  if (Raw->format != STRFMT_CMR) return 0;

  if (!(Cmr = (cmr_t *)calloc(1, sizeof(cmr_t)))) {
    tracet(0,
           "CMR: init_cmr(); unable to allocate CMR dependent private data "
           "structure.\n");
    return 0;
  }
  ...
  if (!(Buffer =
            (unsigned char *)calloc(BUFFER_LENGTH, sizeof(unsigned char)))) {
  ...
  if (!(MessageBuffer = (unsigned char *)calloc(MESSAGEBUFFER_LENGTH,
  ...
  if (!(T4Data = (obsbd_t *)calloc(MAXOBS, sizeof(obsbd_t)))) {
    tracet(0,
           "CMR: init_cmr(); unable to allocate high speed GPS observations "
           "reference table.\n");
    free_cmr(Raw);
    return 0;
  }
...
EXPORT int input_cmr(raw_t *Raw, unsigned char Data) {
  ...
  if (!Cmr->MessageBytes) {
    ...
    if (SyncMessage(Cmr, Data)) {
      /* Found one */
      Cmr->MessageLength = 4 + (unsigned char)MessageBuffer[3] +
                           2; /* 4 (header) + length + 2 (trailer) */
  ...
  */
  if (Cmr->MessageBytes < Cmr->MessageLength) return CheckMessageFlags(Raw);
  ...
  */
  if (MessageBuffer[Cmr->MessageLength - 1] != ETX) {
    tracet(2,
           "CMR: Message did not end with an ETX character. Some data lost.\n");
    Cmr->MessageBytes = 0;
    return CheckMessageFlags(Raw);
  }
  ...
  */
  ...
  /* For the RTK monitor */
  if (Raw->outtype)
    sprintf(Raw->msgtype, "CMR: 0x%02X (%4d)", MessageBuffer[2],
            Cmr->MessageLength);
...
extern int update_cmr(raw_t *Raw, rtksvr_t *Svr, obs_t *obs) {
  ...
  for (n = 0; (n < obs->n) && (n < MAXOBS); n++) {
    ...
    if ((Sat < MAXSAT) && (r->rcv != 2) &&
        (timediff(r->time, RoverObsTable[Sat].Time) > 0)) {
...
static int CheckMessageFlags(raw_t *Raw) {
  ...
  if (Cmr->CurrentMessages != Cmr->PreviousMessages) {
    if (Cmr->CurrentMessages &
        (M_MFLAG_LOWBATMSG1 | M_MFLAG_LOWBATMSG2 | M_MFLAG_LOWBATMSG3))
      strcat(msg, "Low battery at the base");

    if (Cmr->CurrentMessages &
        (M_MFLAG_LOWMEMMSG1 | M_MFLAG_LOWMEMMSG2 | M_MFLAG_LOWMEMMSG3)) {
...
static int DecodeCmr(raw_t *Raw) {
  ...
  if (GtimeToDouble(Raw->time) == 0.0) Raw->time = utc2gpst(timeget());

  if (Type < (sizeof(CMRTable) / sizeof(char *)))
    Type_s = (char *)CMRTable[Type];

  if (!Type_s) Type_s = "Unknown";

  tracet(3,
         "CMR: Trimble Packet Type=0x93 (CMR), CMR Type=%u (%s), CMR "
         "Version=%u, Length=%d.\n",
...
static int DecodeCmrPlus(raw_t *Raw) {
  ...
  tracet(
      3,
      "CMR: Trimble Packet Type=0x94 (CMR+), Base Station=%u, Page=%d of %d.\n",
      StationID, Page, Pages);

  if (!CheckStation(Raw, StationID)) return 0;
...
static int DecodeCmrType0(raw_t *Raw) {
  ...
  for (Slot = 0; (Slot < nsat) && (Slot < MAXOBS); Slot++) {
    ...
    if (L1Flags & M_L1_PHASE_VALID) b->L[0] = sbitn(p + 6, 4, 20) / 256.0;
    ...
    if (L1Flags & M_L1_L2_FOLLOWS) {
      ...
      if ((L2Flags & M_L2_CODE_AVAILABLE) && (L2Flags & M_L2_CODE_VALID)) {
        ...
        if (L2Flags & M_L2_WCODE) b->LLI[1] |= 4; /* Tracking encrypted code */
      }
...
static double GtimeToDouble(gtime_t Gtime) { return Gtime.sec + Gtime.time; }
...
static int ReferenceCmrObs(raw_t *Raw, gtime_t Time, unsigned char Type,
  ...
  if ((Type != CMR_TYPE_4) && (P0 != 0.0)) {
    ...
    if (b->L[0] != 0.0) b->L[0] += b->P[0] / L1WaveLength;
    if (b->P[1] != 0.0) b->P[1] += b->P[0];
    if (b->L[1] != 0.0) b->L[1] += b->P[0] / L2WaveLength;
  }

  if (Type == CMR_TYPE_0) memcpy(&t4[b->Slot], b, sizeof(obsbd_t));
...
static gtime_t ReferenceCmrTime(gtime_t CmrTime, gtime_t RoverTime,
  ...
  return DoubleToGtime((modtime - fmod(modtime, WindowSize)) + CmrTime.time +
                       CmrTime.sec);
}
...
static int sbitn(const unsigned char *Address, int BitPosition, int BitLength) {
  ...
  if ((BitLength == 0) || (32 <= BitLength) ||
      !(Number & (1 << (BitLength - 1))))
    return (int)Number;
...
static void SetStationCoordinates(raw_t *Raw, unsigned char *p) {
  ...
  tracet(3,
         "CMR: Reference station coordinates received. X=%f, Y=%f, Z=%f, East "
         "offset=%f, North offset=%f, Up offset=0.0, Height=%f\n",
         sta->pos[0], sta->pos[1], sta->pos[2], sta->del[0], sta->del[1],
         sta->hgt);
}

/* SetStationDescription - Set the station description */
static void SetStationDescription(raw_t *Raw, unsigned char *p, size_t Length) {
  /* Set the station name with any leading or trailing nulls & white space
   * trimmed off */
  sta_t *sta = &Raw->sta;
...
static void SetStationInfo(raw_t *Raw, unsigned char *p) {
  ...
  tracet(3,
         "CMR: Reference station information received. RECEIVER=\"%s\", "
         "ANTENNA=\"%s\"\n",
...
/* SyncMessage - Synchronize the CMR data stream to the start of a series of CMR
 * messages */
static int SyncMessage(cmr_t *Cmr, unsigned char Data) {
  ...
  | Byte 3 = data length which must be non-zero for any message we're interested
  in.
  */
  return ((MessageBuffer[0] == STX) && (Data != 0) &&
          ((Type == CMR) || (Type == CMRPLUS)));
}
...
static size_t TrimCopy(char *Destination, size_t DestinationLength,
  ...
  */
  for (; (SourceLength > 0) && ((*Source == 0) || isspace(*Source));
       Source++, SourceLength--)
    ;
  for (e = Source + SourceLength - 1;
       (SourceLength > 0) && ((*e == 0) || isspace(*e)); e--, SourceLength--)
    ;

  /* Only copy as many characters as we actually have */
  if (DestinationLength > SourceLength) DestinationLength = SourceLength;
...
static unsigned int ubitn(const unsigned char *Address, int BitPosition,
  ...
  if ((BitPosition < 0) || (BitPosition > 31) || (BitLength <= 0) ||
      (BitLength > 32)) {
    tracet(
        0,
        "CMR: internal error; ubitn() bit position:length violation %u:%u.\n",
