...
static const char rcsid[] =
    "$Id: solution.c,v 1.1 2008/07/17 21:48:06 ttaka Exp $";
...
FILE *fp_GT = NULL;     /* ground truth file pointer */
char GTpath[1024] = ""; /* ground truth file original path */
ecef_pnt_t *GTs;
int nGt = 0;
...
static int decode_nmearmc(char **val, int n, sol_t *sol) {
  ...
  trace(5,
        "decode_nmearmc: %s rr=%.3f %.3f %.3f stat=%d ns=%d vel=%.2f dir=%.0f "
        "ang=%.0f mew=%c mode=%c\n",
        time_str(sol->time, 0), sol->rr[0], sol->rr[1], sol->rr[2], sol->stat,
        sol->ns, vel, dir, ang, mew, mode);
...
static int decode_nmeagga(char **val, int n, sol_t *sol) {
  ...
  trace(5,
        "decode_nmeagga: %s rr=%.3f %.3f %.3f stat=%d ns=%d hdop=%.1f ua=%c "
        "um=%c\n",
        time_str(sol->time, 0), sol->rr[0], sol->rr[1], sol->rr[2], sol->stat,
        sol->ns, hdop, ua, um);
...
static char *decode_soltime(char *buff, const solopt_t *opt, gtime_t *time) {
  ...
  if (sscanf(buff, "%lf/%lf/%lf %lf:%lf:%lf", v, v + 1, v + 2, v + 3, v + 4,
             v + 5) >= 6) {
    if (v[0] < 100.0) {
  ...
  if (n >= 2 && 0.0 <= v[0] && v[0] <= 3000.0 && 0.0 <= v[1] &&
      v[1] < 604800.0) {
...
static void readsolopt(FILE *fp, solopt_t *opt) {
  ...
  for (i = 0; fgets(buff, sizeof(buff), fp) && i < 100;
       i++) { /* only 100 lines */
...
extern int inputsol(unsigned char data, gtime_t ts, gtime_t te, double tint,
  ...
  if (stat != 1 || !screent(sol.time, ts, te, tint) ||
      (qflag && sol.stat != qflag)) {
    return 0;
  }
...
static int sort_solbuf(solbuf_t *solbuf) {
  ...
  if (!(solbuf_data =
            (sol_t *)realloc(solbuf->data, sizeof(sol_t) * solbuf->n))) {
...
/* read groundtruth data from groundtruth file ---------------------------------
 *-----------------------------------------------------------------------------*/
extern int readgt(const char *file) {
  char *efiles[MAXEXFILE] = {0};
  FILE *fp;
  char buff[256]; /* message buffer */
  int n;
  double val[4] = {0};
  ...
  if (!(GTs = (obsd_t *)realloc(GTs, sizeof(obsd_t) * NINCOBS))) {
    trace(1, "addobsdata: memalloc error n=%dx%d\n", sizeof(obsd_t), NINCOBS);
    free(GTs);
    GTs = NULL;
    return -1;
  }
  ...
  while (fgets(buff, sizeof(buff), fp)) {
    /* input solution */
    if ((n = tonum(buff, " ", val)) < 3) return 0;
    GTs[nGt].tow = val[0];
    for (int j = 0; j < 3; j++) {
      GTs[nGt].xyz[j] = val[j + 1]; /* xyz */
    }
    nGt++;
  }
...
extern int addsol(solbuf_t *solbuf, const sol_t *sol) {
  ...
  if (solbuf->n >= solbuf->nmax) {
    ...
    if (!(solbuf_data =
              (sol_t *)realloc(solbuf->data, sizeof(sol_t) * solbuf->nmax))) {
...
static int sort_solstat(solstatbuf_t *statbuf) {
  ...
  if (!(statbuf_data =
            realloc(statbuf->data, sizeof(solstat_t) * statbuf->n))) {
...
static int decode_solstat(char *buff, solstat_t *stat) {
  ...
  n = sscanf(buff, "$SAT%d%lf%s%d%lf%lf%lf%lf%d%d%d%d%d%d%d%d", &week, &tow, id,
             &frq, &az, &el, &resp, &resc, &vsat, &snr, &fix, &slip, &lock,
             &outc, &slipc, &rejc);
...
static void addsolstat(solstatbuf_t *statbuf, const solstat_t *stat) {
  ...
  if (statbuf->n >= statbuf->nmax) {
    ...
    if (!(statbuf_data = (solstat_t *)realloc(
              statbuf->data, sizeof(solstat_t) * statbuf->nmax))) {
...
static int outecef(unsigned char *buff, const char *s, const sol_t *sol,
                   const solopt_t *opt, const double *err_enu) {
  const char *sep = opt2sep(opt);
  char *p = (char *)buff;
  ...
  // original
  /*p += sprintf(p,
     "%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f\n",
          s, sep, sol->rr[0], sep, sol->rr[1], sep, sol->rr[2], sep, sol->stat,
     sep, sol->ns, sep, SQRT(sol->qr[0]), sep, SQRT(sol->qr[1]), sep,
     SQRT(sol->qr[2]), sep, sqvar(sol->qr[3]), sep, sqvar(sol->qr[4]), sep,
     sqvar(sol->qr[5]), sep, sol->age, sep, sol->ratio);*/
  // full
  // p += sprintf(
  //     p,
  //     "%s%s%14.4f%s%14.4f%s%14.4f%s%14.4f%s%14.4f%s%14.4f%s%14.4f%s%10.4f%s%10."
  //     "4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f\n",
  //     s, sep, sol->rr[0], sep, sol->rr[1], sep, sol->rr[2], sep, sol->dtr[0],
  //     sep, sol->dtr[1], sep, sol->dtr[2], sep, sol->dtr[3], sep, sol->dtrp, sep,
  //     sol->dop[1], sep, sol->stat, sep, sol->ns, sep, SQRT(sol->qr[0]), sep,
  //     SQRT(sol->qr[1]), sep, SQRT(sol->qr[2]), sep, sqvar(sol->qr[3]), sep,
  //     sqvar(sol->qr[4]), sep, sqvar(sol->qr[5]), sep, sol->age, sep,
  //     sol->ratio);
  // t-xyz-err_enu
  p += sprintf(
      p,
      "%s%s%14.4f%s%14.4f%s%14.4f%s%14.4f%s%14.4f%s%14.4f\n",
      s, sep, sol->rr[0], sep, sol->rr[1], sep, sol->rr[2], sep, err_enu[0],
      sep, err_enu[1], sep, err_enu[2]);
  return p - (char *)buff;
}
...
static int outpos(unsigned char *buff, const char *s, const sol_t *sol,
  ...
  if (opt->degf) {
    ...
    p += sprintf(p, "%s%s%4.0f%s%02.0f%s%08.5f%s%4.0f%s%02.0f%s%08.5f", s, sep,
                 dms1[0], sep, dms1[1], sep, dms1[2], sep, dms2[0], sep,
                 dms2[1], sep, dms2[2]);
  } else
    p += sprintf(p, "%s%s%14.9f%s%14.9f%s%10.4f%s%14.4f%s%14.4f%s%14.4f\n", s,
                 sep, pos[0] * R2D, sep, pos[1] * R2D, sep, pos[2], sep,
                 err_enu[0], sep, err_enu[1], sep, err_enu[2]);
  // p += sprintf(p,
  //              "%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%"
  //              "6.2f%s%6.1f\n",
  //              sep, sol->stat, sep, sol->ns, sep, SQRT(Q[4]), sep,
...
static int outenu(unsigned char *buff, const char *s, const sol_t *sol,
  ...
  ecef2enu(pos, rr, enu);
  p += sprintf(p,
               "%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s%8."
               "4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f\n",
               s, sep, enu[0], sep, enu[1], sep, enu[2], sep, sol->stat, sep,
               sol->ns, sep, SQRT(Q[0]), sep, SQRT(Q[4]), sep, SQRT(Q[8]), sep,
               sqvar(Q[1]), sep, sqvar(Q[5]), sep, sqvar(Q[2]), sep, sol->age,
               sep, sol->ratio);
  return p - (char *)buff;
}
...
extern int outnmea_rmc(unsigned char *buff, const sol_t *sol) {
  ...
  p += sprintf(
      p,
      "$GPRMC,%02.0f%02.0f%05.2f,A,%02.0f%010.7f,%s,%03.0f%010.7f,%s,%4.2f,%4."
      "2f,%02.0f%02.0f%02d,%.1f,%s,%s",
      ep[3], ep[4], ep[5], dms1[0], dms1[1] + dms1[2] / 60.0,
      pos[0] >= 0 ? "N" : "S", dms2[0], dms2[1] + dms2[2] / 60.0,
      pos[1] >= 0 ? "E" : "W", vel / KNOT2M, dir, ep[2], ep[1],
      (int)ep[0] % 100, amag, emag,
      sol->stat == SOLQ_DGPS || sol->stat == SOLQ_FLOAT || sol->stat == SOLQ_FIX
          ? "D"
          : "A");
...
extern int outnmea_gga(unsigned char *buff, const sol_t *sol) {
  ...
  deg2dms(fabs(pos[1]) * R2D, dms2, 7);
  p += sprintf(p,
               "$GPGGA,%02.0f%02.0f%05.2f,%02.0f%010.7f,%s,%03.0f%010.7f,%s,%d,"
               "%02d,%.1f,%.3f,M,%.3f,M,%.1f,",
               ep[3], ep[4], ep[5], dms1[0], dms1[1] + dms1[2] / 60.0,
               pos[0] >= 0 ? "N" : "S", dms2[0], dms2[1] + dms2[2] / 60.0,
               pos[1] >= 0 ? "E" : "W", solq, sol->ns, dop, pos[2] - h, h,
               sol->age);
  for (q = (char *)buff + 1, sum = 0; *q; q++) sum ^= *q; /* check-sum */
  p += sprintf(p, "*%02X%c%c", sum, 0x0D, 0x0A);
  return p - (char *)buff;
...
extern int outprcopts(unsigned char *buff, const prcopt_t *opt) {
  const int sys[] = {SYS_GPS, SYS_GLO, SYS_GAL, SYS_QZS,
                     SYS_CMP, SYS_IRN, SYS_SBS, 0};
  const char *s1[] = {"single",      "dgps",  "kinematic",     "static",
  ...
  const char *s4[] = {"off",        "broadcast", "sbas", "iono-free",
                      "estimation", "ionex tec", "qzs",  "lex",
                      "vtec_sf",    "vtec_ef",   "gtec", ""};
  const char *s5[] = {"off",     "saastamoinen", "sbas",
                      "est ztd", "est ztd+grad", ""};
  const char *s6[] = {"broadcast",
                      "precise",
                      "broadcast+sbas",
                      "broadcast+ssr apc",
                      "broadcast+ssr com",
                      "qzss lex",
                      ""};
  const char *s7[] = {"gps",    "glonass", "galileo", "qzss",
                      "beidou", "irnss",   "sbas",    ""};
...
extern int outsolheads(unsigned char *buff, const solopt_t *opt) {
  ...
  if (opt->posf == SOLF_NMEA || opt->posf == SOLF_STAT ||
      opt->posf == SOLF_GSIF) {
    return 0;
  }
  if (opt->outhead) {
    ...
    p += sprintf(
        p,
        ",Q=1:fix,2:float,3:sbas,4:dgps,5:single,6:ppp,ns=# of satellites)\n");
  }
  p += sprintf(p, "%s  %-*s%s", COMMENTH, (opt->timef ? 16 : 8) + timeu + 1,
               s3[opt->times], sep);

  if (opt->posf == SOLF_LLH) { /* lat/lon/hgt */
    if (opt->degf) {
      p += sprintf(p,
                   "%16s%s%16s%s%10s%s%3s%s%3s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s%s%"
                   "6s%s%6s\n",
                   "latitude(d'\")", sep, "longitude(d'\")", sep, "height(m)",
                   sep, "Q", sep, "ns", sep, "sdn(m)", sep, "sde(m)", sep,
                   "sdu(m)", sep, "sdne(m)", sep, "sdeu(m)", sep, "sdue(m)",
                   sep, "age(s)", sep, "ratio");
    } else {
      p += sprintf(p,
                   "%14s%s%14s%s%10s%s%3s%s%3s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s%s%"
                   "6s%s%6s\n",
                   "latitude(deg)", sep, "longitude(deg)", sep, "height(m)",
                   sep, "Q", sep, "ns", sep, "sdn(m)", sep, "sde(m)", sep,
                   "sdu(m)", sep, "sdne(m)", sep, "sdeu(m)", sep, "sdun(m)",
                   sep, "age(s)", sep, "ratio");
    }
  } else if (opt->posf == SOLF_XYZ) { /* x/y/z-ecef */
    p += sprintf(
        p,
        "%14s%s%14s%s%14s%s%3s%s%3s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s%s%6s%s%6s\n",
    ...
    p += sprintf(
        p,
        "%14s%s%14s%s%14s%s%3s%s%3s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s%s%6s%s%6s\n",
        "e-baseline(m)", sep, "n-baseline(m)", sep, "u-baseline(m)", sep, "Q",
        sep, "ns", sep, "sde(m)", sep, "sdn(m)", sep, "sdu(m)", sep, "sden(m)",
        sep, "sdnu(m)", sep, "sdue(m)", sep, "age(s)", sep, "ratio");
  }
  return p - (char *)buff;
}
/* std-dev of soltuion -------------------------------------------------------*/
...
extern int outsols(unsigned char *buff, const sol_t *sol, const double *rb,
  ...
  if (opt->timef)
    ...
    sprintf(s, "%*.*f", 6 + (timeu <= 0 ? 0 : timeu + 1), timeu,
            gpst);
  }

  // output error
  double Tm = atof(s), err_enu[3] = {0.0};
  for (size_t i = 0; i < nGt; i++) {
    ecef_pnt_t GT = GTs[i];
    if (fabs(GT.tow - Tm) < 1.0e-4) {
      double err[3] = {0.0}, base_pos[3] = {0.0};
      for (int j = 0; j < 3; j++) err[j] = sol->rr[j] - GT.xyz[j];
      ecef2pos(rb, base_pos);
      ecef2enu(base_pos, err, err_enu);
      break;
    }
  }
  ...
  switch (opt->posf) {
    case SOLF_LLH:
      p += outpos(p, s, sol, opt, err_enu);
      break;
    case SOLF_XYZ:
      p += outecef(p, s, sol, opt, err_enu);
      break;
    case SOLF_ENU:
