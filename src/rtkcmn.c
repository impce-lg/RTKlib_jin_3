 ...
 *     [3] M.Rothacher, R.Schmid, ANTEX: The Antenna Exchange Format
 *Version 1.4, 15 September, 2010 [4] A.Gelb ed., Applied Optimal Estimation,
 *The M.I.T Press, 1974 [5] A.E.Niell, Global mapping functions for the
 *atmosphere delay at radio wavelengths, Jounal of geophysical research, 1996
 ...
 *     [10] GLONASS/GPS/Galileo/Compass/SBAS NV08C receiver series BINR
 *interface protocol specification ver.1.3, August, 2012
 *
...
#define NX_kind 9 /* NX kind: pos,vel,acc,rcvclk,isb,trp,iono,dcb,amb */
...
static const double gst0[] = {1999, 8, 22,
...
const double lam_carr[MAXFREQ] = {/* carrier wave length (m) */
                                  ...
                                  CLIGHT / FREQ5, CLIGHT / FREQ6,
                                  CLIGHT / FREQ7, CLIGHT / FREQ8,
                                  CLIGHT / FREQ9};
const prcopt_t prcopt_default = {
    /* defaults processing options */
    {2000, 1, 1, 0, 0, 0}, {2000, 1, 1, 23, 59, 30}, /* sTimeStart, sTimeEnd */
    PMODE_SINGLE, 0, 2, SYS_GPS, /* mode,soltype,nf,navsys */
    15.0 * D2R, {{0, 0}}, /* elmin,snrmask */
    0, 1, 1, 1, /* sateph,modear,glomodear,bdsmodear */
    5, 0, 10, 1, /* maxout,minlock,minfix,armaxiter */
    0, 0, 0, 1, /* estion,esttrop,dynamics,tidecorr */
    1, 0, 0, 0, 0, /* niter,codesmooth,intpref,sbascorr,sbassatsel */
    0, 0,                                        /* rovpos,refpos */
    {100.0, 100.0},                           /* eratio[] */
    {100.0, 0.003, 0.003, 0.0, 1.0},          /* err[] */
    {30.0, 0.03, 0.3},                        /* std[] */
    {1E-4, 1E-3, 1E-4, 1E-1, 1E-2, 0.0, 0.0}, /* prn[] */
    5E-12,                                    /* sclkstab */
    {3.0, 0.9999, 0.15, 0.15, 0.05},          /* thresar */
    0.0, 0.0, 0.05, /* elmaskar,almaskhold,thresslip */
    30.0, 30.0, 30.0, /* maxtdif,maxinno,maxgdop */
    {0}, {0}, {0},      /* baseline,ru,rb */
    {"", ""}, /* anttype */
    {{0}}, {{0}}, {0}, /* antdel,pcv,exsats */
    1, 1 /* ppp adaptive,robust kalman filter */
};
const solopt_t solopt_default = {
    /* defaults solution output options */
    SOLF_LLH,   TIMES_GPST, 1,  3, /* posf,times,timef,timeu */
    0, 1, 0,  0, 0, 0, /* degf,outhead,outopt,datum,height,geoid */
    0, 0, 0,  0, 0, 5, /* solstatic,samb,sres,sazl,sstat,trace */
    {0.0, 0.0}, /* nmeaintv */
...
extern int satno(int sys, int prn) {
  ...
  switch (sys) {
    ...
    case SYS_IRN:
      ...
      return NSATGPS + NSATGLO + NSATGAL + NSATQZS + NSATCMP + prn - MINPRNIRN +
             1;
    case SYS_LEO:
      if (prn < MINPRNLEO || MAXPRNLEO < prn) return 0;
      return NSATGPS + NSATGLO + NSATGAL + NSATQZS + NSATCMP + NSATIRN + prn -
             MINPRNLEO + 1;
    case SYS_SBS:
 ...
 * args   : char   *id       I   satellite id (nn,Gnn,Rnn,Enn,Jnn,Cnn,Inn or
 *Snn) return : satellite number (0: error) notes  : 120-142 and 193-199 are
 *also recognized as sbas and qzss
 *-----------------------------------------------------------------------------*/
...
extern int getcodepri(int sys, unsigned char code, const char *opt) {
  ...
  auto Po = strchr(codepris[i][j - 1], obs[1]);
...
/* trace of matrix -----------------------------------------------------------*/
extern double mattrc(double *A, int m, int n) {
  double A_trc = 0.0;
  int i;

  for (i = 0; i < m; i++) A_trc += A[i + i * n];
  return A_trc;
}
/* inverse of matrix ---------------------------------------------------------*/
extern int matinv(double *A, int n) {
 ...
 * return : status (0:ok,0>:error)
 * notes  : for weighted least square, replace A and y by A*w and w*y
 *(w=W^(1/2)) matirix stored by column-major order (fortran convention)
 *-----------------------------------------------------------------------------*/
...
/* robust adaptive factor(s) construction ------------------------------------*/
/* bubble sort */
extern void bubblesort(double *list, int len) {
  int i, j;
  double temp;
  for (i = 0; i < len - 1; i++)
    for (j = 0; j < len - i - 1; j++) {
...
/* Hampel median function for robust factors */
static double Hampel(const double *x, int m) {
  double *x_, x_med, sigma = 1.0;
  int i;

  x_ = mat(m, 1);
  for (i = 0; i < m; i++) x_[i] = fabs(x[i]);

  bubblesort(x_, m);

  if (m % 2 == 0)
    x_med = (x_[m / 2] + x_[m / 2 - 1]) / 2.0;
  else
    x_med = x_[m / 2];

  sigma = fabs(x_med) / 0.6475;
  free(x_);

  return sigma;
}
/* Huber dual segment function for adaptive/robust factor(s) */
static double Huber(double x, double c) {
  double factor = 1.0;
...
/* triple segment function for adaptive/robust factor(s) */
static double segmentf3(double x, double c0, double c1) {
  double factor = 1.0;

  if (x <= 0.0 || c0 <= 0.0 || c1 <= 0.0)
    return;
  else if (x > c0 && x <= c1)
    factor = (c0 / x) * SQR((c1 - x) / (c1 - c0));
  else if (x > c1)
    factor = 1.0E-5;

  return factor;
}
/* robust factors  constrution with measurement residual: v=Z-H*X^
 * X^   : updated state with kalman filter previously
 *        X^ derives from both measurement and predicted state
 * reference :
 *    [1] Gao zhou zheng, PhD thesis */
extern int robust(const double *H, const double *Pp, const double *v,
                  const double *xp, int n, int m, const prcopt_t *opt,
                  double *R) {
  double c = 1.0, c0 = 1.5, c1 = 5.0, *H_, H_row_prd, *Pp_, sigma_v_;
  double *v_, *F, *Pv_, *robust_factor;
  int i, j, k, *ix, info = 0;

  ix = imat(n, 1);
  for (i = k = 0; i < n; i++) {
    H_row_prd = 0.0;
    for (j = 0; j < m; j++) H_row_prd += SQR(H[i + j * n]);
    if (xp[i] != 0.0 && Pp[i + i * n] > 0.0 && H_row_prd > 0.0) ix[k++] = i;
  }
  if (k > m) return 1;
  H_ = mat(k, m);
  Pp_ = mat(k, k);
  ...
  F = mat(k, m);
  Pv_ = mat(m, m);
  robust_factor = mat(m, 1);

  for (i = 0; i < k; i++) {
    for (j = 0; j < k; j++) Pp_[i + j * k] = Pp[ix[i] + ix[j] * n];
    for (j = 0; j < m; j++) H_[i + j * k] = H[ix[i] + j * n];
  }
  matcpy(Pv_, R, m, m);
  matmul("NN", k, m, k, 1.0, Pp_, H_, 0.0, F); /* Pv_=H'*Pp*H+R */
  matmul("TN", m, m, k, 1.0, H_, F, 1.0, Pv_);

  trace(6, "v=:\n");
  tracemat(6, v, 1, m, 12, 5);
  trace(6, "Pv_=:\n");
  tracemat(6, Pv_, m, m, 12, 5);
  /* construct robust factor */
  /* Huber/segmentf3 function */
  for (i = 0; i < m; i++) {
    v_[i] = fabs(v[i] / SQRT(Pv_[i + i * m]));
    robust_factor[i] = Huber(fabs(v[i] / SQRT(Pv_[i + i * m])), c);
    if (robust_factor[i] != 1.0) {
      R[i + i * m] /= robust_factor[i];
    }
  }

  trace(6, "Std res v_=:\n");
  tracemat(6, v_, 1, m, 12, 5);
  trace(6, "robust_factor=:\n");
  tracemat(6, robust_factor, 1, m, 12, 5);
  free(H_);
  free(Pp_);
  free(F);
  free(Pv_);
...
/* classified robust factors  constrution with measurement residual: v=L-H*X~
* X~   : updated state increment with lsq within this function
* L    : computation residual
         L=Z-H*X_(Z: measurement, X_:initial state vector)
*        X~ =(H*R*H')^-1*H*R*L
* reference :
*    [1] Yang Y, Cui X. Adaptively robust filter with multi adaptive factors[J].
         Survey Review, 2008, 40(309):260-270.*/
extern int robust2(const double *H, const double *Pp, const double *L,
                   const double *xp, int n, int m, const prcopt_t *opt,
                   double *R) {
  double c = 1.0, c0 = 1.5, c1 = 20.0, H_row_prd, *H_, *R_, *L_, sig;
  double *He, *Xe, *PXe, *Le, *v, *v_, *F, *Pv_, sigma_v, *robust_factor;
  int nr, nr_, i, j, k, kk, *ix, m_, info = 0;

  /* to identify whether the obs num of phase equals to code */
  if ((m % 2))
    return 1;
  else
    m_ = m / 2;
  ...
  ix = imat(nr, 1);
  for (i = nr_ = 0; i < nr; i++) {
    H_row_prd = 0.0;  // inner product of the ith row of H
    for (j = 0; j < m; j++) H_row_prd += SQR(H[i + j * n]);
    if (xp[i] != 0.0 && Pp[i + i * n] > 0.0 && H_row_prd > 0.0) ix[nr_++] = i;
  }
  if (nr_ >= m_) return 1;

  // trace(6, "R=:\n"); tracemat(6, R, m, m, 12, 5);
  for (i = 0; i < 2; i++) {
    H_ = mat(nr_, m_);
    R_ = zeros(m_, m_);
    L_ = mat(m_, 1);
    ...
    F = mat(nr_, m_);
    Pv_ = mat(m_, m_);
    robust_factor = mat(m_, 1);

    for (j = 0; j < nr_; j++) {
      for (k = 0, kk = i; k < m_, kk < m; k++, kk = kk + 2)
        H_[j + k * nr_] = H[ix[j] + kk * n];
    }
    for (j = 0; j < m_; j++) {
      R_[j + j * m_] = R[(2 * j + i) + (2 * j + i) * m];
      L_[j] = L[2 * j + i];
    }
    matcpy(He, H_, nr_, m_);
    matcpy(Le, L_, m_, 1);

    // trace(6, "Le_%s=:\n", i == 0 ? "L" : "P"); tracemat(6, Le, 1, m_, 12, 5);
    /*trace(6, "H_=:\n"); tracemat(6, H_, nr_, m_, 12, 5);
    trace(6, "R_=:\n"); tracemat(6, R_, m_, m_, 12, 5);*/

    /* weight by variance */
    for (j = 0; j < m_; j++) {
      sig = SQRT(R_[j + j * m_]);
      Le[j] /= sig;
      for (k = 0; k < nr_; k++) He[k + j * nr_] /= sig;
    }
    /* least square estimation of x, using measurements only */
    if (info = lsq(He, Le, nr_, m_, Xe, PXe)) {
      if (info == -1) return info;
    }
    matcpy(v, L_, m_, 1);
    matmul("TN", m_, 1, nr_, 1.0, H_, Xe, -1.0, v); /* v  = H'* Xe- L */
    matcpy(Pv_, R_, m_, m_);
    matmul("NN", nr_, m_, nr_, 1.0, PXe, H_, 0.0, F); /* Pv_= H'* PXe * H + R */
    matmul("TN", m_, m_, nr_, 1.0, H_, F, 1.0, Pv_);

    trace(6, "v%s=:\n", i == 0 ? "L" : "P");
    tracemat(6, v, 1, m_, 12, 5);
    // trace(6, "Pv_=:\n"); tracemat(6, Pv_, m_, m_, 12, 5);
    /* construct robust factor */
    for (j = 0; j < m_; j++) {
      v_[j] = fabs(v[j] / SQRT(Pv_[j + j * m_]));
      robust_factor[j] = Huber(fabs(v[j] / SQRT(Pv_[j + j * m_])), c);
      if (robust_factor[j] != 1.0) {
        R[(2 * j + i) + (2 * j + i) * m] /= robust_factor[j];
      }
    }

    ///* Hample median function */
    // sigma_v = Hampel(v, m_);
    // for (j = 0; j < m_; j++)
    //{
    //	v_[j] = fabs(v[j] / sigma_v);
    //	robust_factor[j] = c / fabs(v[j] / sigma_v);
    //	R[(2 * j + i) + (2 * j + i)*m] /= robust_factor[j];
    //}

    // trace(6, "Std res v_%s=:\n", i == 0 ? "L" : "P"); tracemat(6, v_, 1, m_,
    // 12, 5); trace(6, "robust_factor=:\n"); tracemat(6, robust_factor, 1, m_,
    // 12, 5);
...
/* classified adaptively factors construction
 * reference :
 *    [1] Cui X, Yang Y. Adaptively robust filtering with classified adaptive
 * factors[J]. Progress in Natural Science:Materials
 * International(��Ȼ��ѧ��չ:���ʲ���(Ӣ��)), 2006, 16(8):846-851.*/
extern int adapt(const double *H, const double *R, const double *v,
                 const double *xp, int n, int m, prcopt_t *opt, double *P) {
  double c = 1.0, c0 = 1.5, c1 = 3.0, sig, *H_, H_row_prd, *v_, *Q, *P_;
  double *dx, *dx_, *dx_smod, *adapt_factor;
  int i, j, k, *ix, info = 0;
  ...
  for (i = k = 0; i < n; i++) {
    H_row_prd = 0.0;
    for (j = 0; j < m; j++) H_row_prd += SQR(H[i + j * n]);
    if (xp[i] != 0.0 && P[i + i * n] > 0.0 && H_row_prd > 0.0) ix[k++] = i;
  }
  if (k >= m) return 1;
  ...
  dx = zeros(n, 1);
  dx_ = mat(k, 1);
  dx_smod = zeros(NX_kind, 1);
  adapt_factor = zeros(NX_kind, 1);
  matcpy(v_, v, m, 1);
  ...
  /* least square estimation of x, using measurements only */
  if (info = lsq(H_, v_, k, m, dx_, Q)) {
    if (info == -1) return info;
  }
  for (i = 0; i < k; i++) dx[ix[i]] = dx_[i];
  /* construct adaptive factor and update predicted state covariance matrix Pp*/
  /* position */
  if (dot(dx, dx, 3) != 0.0) {
    dx_smod[0] = SQRT(dot(dx, dx, 3) / mattrc(P, 3, n));
    adapt_factor[0] = Huber(dx_smod[0], c);
    if (adapt_factor[0] != 1.0) {
      for (j = 0; j < 3; j++) P[j + j * n] /= adapt_factor[0];
    }
  }
  /* velocity and acceleration */
  if (opt->dynamics) {
    for (i = 1; i < 3; i++) {
      if ((dot(dx, dx, 3 * (i + 1)) - dot(dx, dx, 3 * i)) != 0.0) {
        dx_smod[i] = SQRT((dot(dx, dx, 3 * (i + 1)) - dot(dx, dx, 3 * i)) /
                          (mattrc(P, 3 * (i + 1), n) - mattrc(P, 3 * i, n)));
        adapt_factor[i] = Huber(dx_smod[i], c);
        if (adapt_factor[i] != 1.0) {
          for (j = 3 * i; j < 3 * (i + 1); j++) P[j + j * n] /= adapt_factor[i];
        }
      }
    }
  }
  /* receiver clock */
  if (dx[IC(0, opt)] != 0.0) {
    dx_smod[3] = fabs(dx[IC(0, opt)]) / SQRT(P[IC(0, opt) + IC(0, opt) * n]);
    adapt_factor[3] = Huber(dx_smod[3], c);
    if (adapt_factor[3] != 1.0)
      P[IC(0, opt) + IC(0, opt) * n] /= adapt_factor[3];
  }
  ///* isb */
  // if ((dot(dx, dx, IC(NSYS, opt)) - dot(dx, dx, IC(1, opt))) != 0.0)
  //{
  //	dx_smod[4] = SQRT((dot(dx, dx, IC(NSYS, opt)) - dot(dx, dx, IC(1, opt)))
  /// 		(mattrc(P, IC(NSYS, opt) , n) - mattrc(P, IC(1, opt), n)));
  //	adapt_factor[4] = Huber(dx_smod[4], c);
  //	if (adapt_factor[4] != 1.0)
  //	{
  //		for (j = IC(1, opt); j < IC(NSYS, opt); j++)
  //			P[j + j*n] /= adapt_factor[4];
  //	}
  //}
  ///* tropsphere */
  // if (dx[IT(opt)] != 0.0)
  //{
  //	dx_smod[5] = fabs(dx[IT(opt)]) / SQRT(P[IT(opt) + IT(opt) * n]);
  //	adapt_factor[5] = Huber(dx_smod[5], c);
  //	if (adapt_factor[5] != 1.0)
  //		P[IT(opt) + IT(opt) * n] /= adapt_factor[5];
  //}
  ///* ambiguity */
  // if ((dot(dx, dx, n) - dot(dx, dx, NR(opt))) != 0.0)
  //{
  //	dx_smod[8] = SQRT((dot(dx, dx, n) - dot(dx, dx, NR(opt))) /
  //		(mattrc(P, n, n) - mattrc(P, NR(opt), n)));
  //	adapt_factor[8] = Huber(dx_smod[8], c);
  //	if (adapt_factor[8] != 1.0)
  //	{
  //		for (j = NR(opt); j < n; j++)
  //			P[j + j*n] /= adapt_factor[8];
  //	}
  //}
  if (dot(adapt_factor, adapt_factor, NX_kind) -
      (int)dot(adapt_factor, adapt_factor, NX_kind))
    trace(6, "adapt_factor=:\n");
  tracemat(6, adapt_factor, 1, NX_kind, 12, 5);
 ...
 *          double *Pp       O   covariance matrix of states after update (n x
 *n) return : status (0:ok,<0:error) notes  : matirix stored by column-major
 *order (fortran convention) if state x[i]==0.0, not updates state x[i]/P[i+i*n]
 *-----------------------------------------------------------------------------*/
static int filter_(const double *x, const double *P, const double *H,
  ...
  if (!(info = matinv(Q, m))) {
    ...
    double Kv[n];
    for (int i = 0; i < n; i++) Kv[i] = xp[i] - x[i];

    matmul("NT", n, n, m, -1.0, K, H, 1.0, I); /* Pp=(I-K*H')*P */
    trace(5, "I-K*H'(%d*%d)=:\n", n, n); tracemat(5, I, n, n, 12, 5);
    matmul("NN", n, n, n, 1.0, I, P, 0.0, Pp);

    trace(5, "P_(%d*%d)=:\n", n, n); tracemat(5, P, n, n, 12, 5);
    // trace(5, "R(%d*%d)=:\n", m, m); tracemat(5, R, m, m, 12, 5);
    trace(5, "Pp(%d*%d)=:\n", n, n); tracemat(5, Pp, n, n, 12, 5);
  }
  free(F);
  free(Q);
  free(K);
...
extern int filter(double *x, double *P, const double *H, const double *v,
  ...
  /*trace(6, "P_(%d*%d)=:\n", k, k); tracemat(6, P_, k, k, 12, 5);
  trace(6, "R(%d*%d)=:\n", m, m); tracemat(6, R, m, m, 12, 5);
  trace(6, "H_(%d*%d)=:\n", k, m); tracemat(6, H_, k, m, 12, 5);*/
  info = filter_(x_, P_, H_, v, R, k, m, xp_, Pp_);
  /*if (R[0] > 1E-4){ trace(6, "post-filter state variance matrix
   * Pp(float)=:\n"); tracemat(6, Pp_, k, k, 12, 5); }*/
  for (i = 0; i < k; i++) {
    x[ix[i]] = xp_[i];
...
// 	}
// }
// extern void imatprint(const int A[], int n, int m, int p, int q)
// {
// 	imatfprint(A, n, m, p, q, stdout);
// }
/* string to number ------------------------------------------------------------
...
extern int loacaltime(int basetime_hr, double dlon) {
  int localt, t1;
  double rem;

  t1 = (int)(dlon / 15.0 + 1E-6);
  rem = dlon - (double)(t1)*15.0;
  if (fabs(rem) > 7.5) t1 += (int)(rem / 7.5 + 1E-6);

  if ((localt = basetime_hr + t1) < 0) localt += 24;
  return localt;
}
/* add time --------------------------------------------------------------------
...
extern gtime_t timeget(void) {
  ...
  double ep[6] = {0};
#ifdef WIN32
  SYSTEMTIME sGTimeStart;

  GetSystemTime(&sGTimeStart); /* utc */
  ep[0] = sGTimeStart.wYear;
  ep[1] = sGTimeStart.wMonth;
  ep[2] = sGTimeStart.wDay;
  ep[3] = sGTimeStart.wHour;
  ep[4] = sGTimeStart.wMinute;
  ep[5] = sGTimeStart.wSecond + sGTimeStart.wMilliseconds * 1E-3;
#else
  struct timeval tv;
  struct tm *tt;
...
extern void sleepms(int ms) {
  ...
  if (ms < 5)
    ...
    Sleep(ms);
#else
  struct timespec sGTimeStart;
  if (ms <= 0) return;
  sGTimeStart.tv_sec = (time_t)(ms / 1000);
  sGTimeStart.tv_nsec = (long)(ms % 1000 * 1000000);
  nanosleep(&sGTimeStart, NULL);
#endif
}
...
static int readantex(const char *file, pcvs_t *pcvs) {
  ...
  while (fgets(buff, sizeof(buff), fp)) {
    ...
    if (strstr(buff + 60, "TYPE / SERIAL NO")) {
      ...
      if (!str2time(buff, 0, 43, &pcv.sGTimeStart)) continue;
    } else if (strstr(buff + 60, "VALID UNTIL")) {
      if (!str2time(buff, 0, 43, &pcv.sGTimeEnd)) continue;
...
extern pcv_t *searchpcv(int sat, const char *type, gtime_t time,
  ...
  if (sat) { /* search satellite antenna */
    for (i = 0; i < pcvs->n; i++) {
      ...
      if (pcv->sGTimeStart.time != 0 && timediff(pcv->sGTimeStart, time) > 0.0)
        continue;
      if (pcv->sGTimeEnd.time != 0 && timediff(pcv->sGTimeEnd, time) < 0.0)
        continue;
      return pcv;
    }
  } else {
 ...
 *          double *erpv       O   erp values {xp,yp,ut1_utc,lod}
 *(rad,rad,s,s/d) return : status (1:ok,0:error)
 *-----------------------------------------------------------------------------*/
 ...
 *          gtime_t sGTimeStart    I      time start (sGTimeStart.time==0:no
 *screening by sGTimeStart) gtime_t sGTimeEnd    I      time end
 *(sGTimeEnd.time==0:no screening by sGTimeEnd) double  tint  I      time
 *interval (s) (0.0:no screen by tint) return : 1:on condition, 0:not on
 *condition
 *-----------------------------------------------------------------------------*/
extern int screent(gtime_t time, gtime_t sGTimeStart, gtime_t sGTimeEnd,
                   double tint) {
  return (tint <= 0.0 ||
          fmod(time2gpst(time, NULL) + DTTOL, tint) <= DTTOL * 2.0) &&
         (sGTimeStart.time == 0 || timediff(time, sGTimeStart) >= -DTTOL) &&
         (sGTimeEnd.time == 0 || timediff(time, sGTimeEnd) < DTTOL);
}
  ...
  while (fgets(buff, sizeof(buff), fp)) {
    ...
    if (satsys(sat, &prn) == SYS_GLO) {
      ...
      sscanf(p + 1,
             ...
             "%lf,%lf,%lf,%lf",
             &nav->geph[prn - 1].iode, &nav->geph[prn - 1].frq,
             &nav->geph[prn - 1].svh, &nav->geph[prn - 1].sva,
             &nav->geph[prn - 1].age, &toe_time, &tof_time,
             &nav->geph[prn - 1].pos[0], &nav->geph[prn - 1].pos[1],
             &nav->geph[prn - 1].pos[2], &nav->geph[prn - 1].vel[0],
             &nav->geph[prn - 1].vel[1], &nav->geph[prn - 1].vel[2],
             &nav->geph[prn - 1].acc[0], &nav->geph[prn - 1].acc[1],
             &nav->geph[prn - 1].acc[2], &nav->geph[prn - 1].taun,
      ...
      sscanf(p + 1,
             ...
             &nav->eph[sat - 1].sva, &nav->eph[sat - 1].svh, &toe_time,
             &toc_time, &ttr_time, &nav->eph[sat - 1].A, &nav->eph[sat - 1].e,
             &nav->eph[sat - 1].i0, &nav->eph[sat - 1].OMG0,
             &nav->eph[sat - 1].omg, &nav->eph[sat - 1].M0,
             &nav->eph[sat - 1].deln, &nav->eph[sat - 1].OMGd,
             &nav->eph[sat - 1].idot, &nav->eph[sat - 1].crc,
             &nav->eph[sat - 1].crs, &nav->eph[sat - 1].cuc,
             &nav->eph[sat - 1].cus, &nav->eph[sat - 1].cic,
             &nav->eph[sat - 1].cis, &nav->eph[sat - 1].toes,
             &nav->eph[sat - 1].fit, &nav->eph[sat - 1].f0,
             &nav->eph[sat - 1].f1, &nav->eph[sat - 1].f2,
...
extern int savenav(const char *file, const nav_t *nav) {
  ...
  for (i = 0; i < MAXPRNGLO; i++) {
    ...
    fprintf(fp,
            ...
            (int)nav->geph[i].tof.time, nav->geph[i].pos[0],
            nav->geph[i].pos[1], nav->geph[i].pos[2], nav->geph[i].vel[0],
            nav->geph[i].vel[1], nav->geph[i].vel[2], nav->geph[i].acc[0],
            nav->geph[i].acc[1], nav->geph[i].acc[2], nav->geph[i].taun,
            nav->geph[i].gamn, nav->geph[i].dtaun);
  }
  fprintf(fp,
 ...
 *--------------------------------------------------------- free memory for
 *navigation data args   : nav_t *nav    IO     navigation data int   opt     I
 *option (or of followings) (0x01: gps/qzs ephmeris, 0x02: glonass ephemeris,
 *                                0x04: sbas ephemeris,   0x08: precise
 *ephemeris, 0x10: precise clock     0x20: almanac, 0x40: tec data) return :
 *none
 *-----------------------------------------------------------------------------*/
 ...
 *          char   *rpath    O   file path in which keywords replaced (see
 *below) gtime_t time     I   time (gpst)  (time.time==0: not replaced) char
 **rov      I   rover id string        ("": not replaced) char   *base     I
 *base station id string ("": not replaced) return : status (1:keywords
 *replaced, 0:no valid keyword in the path, -1:no valid time) notes  : the
 *following keywords in path are replaced by date, time and name %Y -> yyyy :
 *year (4 digits) (1900-2099) %y -> yy   : year (2 digits) (00-99) %m -> mm   :
 *month           (01-12) %d -> dd   : day of month    (01-31) %h -> hh   :
 *hours           (00-23) %M -> mm   : minutes         (00-59) %S -> ss   :
 *seconds         (00-59) %n -> ddd  : day of year     (001-366) %W -> wwww :
 *gps week        (0001-9999) %D -> d    : day of gps week (0-6) %H -> h    :
 *hour code       (a=0,b=1,c=2,...,x=23) %ha-> hh   : 3 hours (00,03,06,...,21)
 ...
 *          gtime_t sGTimeStart       I   time start (gpst)
 *          gtime_t sGTimeEnd       I   time end   (gpst)
 ...
 *-----------------------------------------------------------------------------*/
extern int reppaths(const char *path, char *rpath[], int nmax,
                    gtime_t sGTimeStart, gtime_t sGTimeEnd, const char *rov,
                    const char *base) {
  gtime_t time;
  ...
  if (sGTimeStart.time == 0 || sGTimeEnd.time == 0 ||
      timediff(sGTimeStart, sGTimeEnd) > 0.0)
    return 0;
  ...
  tow = time2gpst(sGTimeStart, &week);
  time = gpst2time(week, floor(tow / tint) * tint);

  while (timediff(time, sGTimeEnd) <= 0.0 && n < nmax) {
 ...
 *output) (0.0<=azel[0]<2*pi,-pi/2<=azel[1]<=pi/2) return : elevation angle
 *(rad)
 *-----------------------------------------------------------------------------*/
...
extern double ionmodel(gtime_t t, const double *ion, const double *pos,
  ...
  return CLIGHT * f *
         (fabs(x) < 1.57 ? 5E-9 + amp * (1.0 + x * x * (-0.5 + x * x / 24.0))
                         : 5E-9);
}
 ...
 *------------------------------------------------------ compute satellite
 *antenna phase center parameters args   : pcv_t *pcv       I   antenna phase
 *center parameters double nadir     I   nadir angle for satellite (rad) double
 **dant     O   range offsets for each frequency (m) return : none
 *-----------------------------------------------------------------------------*/
