 ...
 *         7 March, 2006
 *     [2] Global Navigation Satellite System GLONASS, Interface Control
 *Document Navigational radiosignal In bands L1, L2, (Edition 5.1), 2008 [3]
 *RTCA/DO-229C, Minimum operational performanc standards for global positioning
 *system/wide area augmentation system airborne equipment, RTCA inc, November
 *28, 2001 [4] RTCM Paper, April 12, 2010, Proposed SSR Messages for SV Orbit
 *Clock, Code Biases, URA [5] RTCM Paper 012-2009-SC104-528, January 28, 2009
 *(previous ver of [4]) [6] RTCM Paper 012-2009-SC104-582, February 2, 2010
 *(previous ver of [4]) [7] European GNSS (Galileo) Open Service Signal In Space
 *Interface Control Document, Issue 1, February, 2010 [8] Quasi-Zenith Satellite
 *System Navigation Service Interface Control Specification for QZSS (IS-QZSS)
 *V1.1, Japan Aerospace Exploration Agency, July 31, 2009 [9] BeiDou navigation
 *satellite system signal in space interface control document open service
...
static double var_uraeph(int ura) {
  const double ura_value[] = {2.4,   3.4,   4.85,   6.85,   9.65,
...
static int satpos_ssr(gtime_t time, gtime_t teph, int sat, const nav_t *nav,
  ...
  *var = var_urassr(ssr->ura);
  ...
  trace(5,
        "satpos_ssr: %s sat=%2d deph=%6.3f %6.3f %6.3f er=%6.3f %6.3f %6.3f "
        "dclk=%6.3f var=%6.3f\n",
        time_str(time, 2), sat, deph[0], deph[1], deph[2], er[0], er[1], er[2],
        dclk, *var);
  ...
  *svh = 0;
  ...
  switch (ephopt) {
    case EPHOPT_BRDC:
      ...
      // case EPHOPT_SBAS  : return satpos_sbas(time,teph,sat,nav,
      // rs,dts,var,svh);
    case EPHOPT_SSRAPC:
    ...
    case EPHOPT_PREC:
      if (!peph2pos(time, sat, nav, 1, rs, dts, var))
        ...
        return 1;
    case EPHOPT_LEX:
      if (!lexeph2pos(time, sat, nav, rs, dts, var))
        break;
      else
        return 1;
  }
...
extern void satposs(gtime_t teph, const obsd_t *obs, int n, const nav_t *nav,
  ...
  for (i = 0; i < n && i < 2 * MAXOBS; i++) {
    ...
    if (!satpos(time[i], teph, obs[i].sat, ephopt, nav, rs + i * 6, dts + i * 2,
                var + i, svh + i)) {
  ...
  for (i = 0; i < n && i < 2 * MAXOBS; i++) {
    trace(4,
          ...
          time_str(time[i], 6), obs[i].sat, rs[i * 6], rs[1 + i * 6],
          rs[2 + i * 6], dts[i * 2] * 1E9, var[i], svh[i]);
  }
}
