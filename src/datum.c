...
static const char rcsid[] =
    "$Id: datum.c,v 1.1 2008/07/17 21:48:06 ttaka Exp $";
...
static int searchprm(double lat, double lon) {
  ...
  m2 = (int)(lon / 7.5);
  lon -= m2 * 7.5;
  code = n1 * 1000000 + m1 * 10000 + n2 * 1000 + m2 * 100 +
...
static int dlatdlon(const double *post, double *dpos) {
  ...
  a = lat / dlat - (int)(lat / dlat);
  c = 1.0 - a;
  b = lon / dlon - (int)(lon / dlon);
  d = 1.0 - b;
  dpos[0] = (db[0][0] * c * d + db[1][0] * a * d + db[0][1] * c * b +
             db[1][1] * a * b) *
            D2R / 3600.0;
  dpos[1] = (dl[0][0] * c * d + dl[1][0] * a * d + dl[0][1] * c * b +
             dl[1][1] * a * b) *
            D2R / 3600.0;
  return 0;
}
...
extern int loaddatump(const char *file) {
  ...
  while (fgets(buff, sizeof(buff), fp) && n < MAXPRM) {
    if (sscanf(buff, "%d %f %f", &prm[n].code, &prm[n].db, &prm[n].dl) >= 3)
      n++;
  }
  fclose(fp);
