 ...
 *           2016/07/23 1.19 add output of received stream to tcp port for
 *serial 2016/08/20 1.20 modify api strsendnmea() 2016/08/29 1.21 fix bug on
 *starting serial thread for windows 2016/09/03 1.22 add ntrip caster functions
...
static serial_t *openserial(const char *path, int mode, char *msg) {
  const int br[] = {300,   600,   1200,  2400,   4800,  9600,
                    19200, 38400, 57600, 115200, 230400};
  serial_t *serial;
  int i, brate = 9600, bsize = 8, stopb = 1, tcp_port = 0;
  char *p, parity = 'N', dev[128], port[128], fctr[64] = "", path_tcp[32],
           msg_tcp[128];
#ifdef WIN32
  ...
  sprintf(dcb, "baud=%d parity=%c data=%d stop=%d", brate, parity, bsize,
          stopb);
  ...
  serial->state = 1;
  if (!(serial->thread =
            CreateThread(NULL, 0, serialthread, serial, 0, NULL))) {
...
/* write serial --------------------------------------------------------------*/
static int writeserial(serial_t *serial, unsigned char *buff, int n,
                       char *msg) {
  int ns;
...
static int readfile(file_t *file, unsigned char *buff, int nmax, char *msg) {
  ...
  if (file->fp_tag) {
    if (file->repmode) { /* slave */
      ...
      t = (unsigned int)((tickget() - file->tick) * file->speed +
                         file->start * 1000.0);
    }
...
static int writefile(file_t *file, unsigned char *buff, int n, char *msg) {
  ...
  if (file->swapintv > 0.0 && file->wtime.time != 0) {
    ...
    /* open new swap file */
    if (floor((tow1 + fswapmargin) / intv) <
        floor((tow2 + fswapmargin) / intv)) {
...
static int setsock(socket_t sock, char *msg) {
  ...
  if (setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, (const char *)&tv,
                 sizeof(tv)) == -1 ||
      setsockopt(sock, SOL_SOCKET, SO_SNDTIMEO, (const char *)&tv,
                 sizeof(tv)) == -1) {
  ...
  if (setsockopt(sock, SOL_SOCKET, SO_RCVBUF, (const char *)&bs, sizeof(bs)) ==
          -1 ||
      setsockopt(sock, SOL_SOCKET, SO_SNDBUF, (const char *)&bs, sizeof(bs)) ==
          -1) {
    tracet(1, "setsock: setsockopt error 2 sock=%d err=%d bs=%d\n", sock,
           errsock(), bs);
    sprintf(msg, "sockopt error: bufsiz");
  }
  if (setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, (const char *)&mode,
                 sizeof(mode)) == -1) {
...
/* non-block accept ----------------------------------------------------------*/
static socket_t accept_nb(socket_t sock, struct sockaddr *addr,
                          socklen_t *len) {
  struct timeval tv = {0};
  fd_set rs;
  int ret;
...
static int accsock(tcpsvr_t *tcpsvr, char *msg) {
  ...
  if ((sock = accept_nb(tcpsvr->svr.sock, (struct sockaddr *)&addr, &len)) ==
      (socket_t)-1) {
    err = errsock();
...
/* write tcp server ----------------------------------------------------------*/
static int writetcpsvr(tcpsvr_t *tcpsvr, unsigned char *buff, int n,
                       char *msg) {
  int i, ns = 0, err;
...
/* write tcp client ----------------------------------------------------------*/
static int writetcpcli(tcpcli_t *tcpcli, unsigned char *buff, int n,
                       char *msg) {
  int ns, err;
...
static int rspntrip_c(ntrip_t *ntrip, char *msg) {
  ...
  if ((p = strstr((char *)ntrip->buff, NTRIP_RSP_SRCTBL))) { /* source table */
    ...
    ntrip->state = 0;
    discontcp(&ntrip->tcp->svr, ntrip->tcp->tirecon);
  } else if ((p = strstr((char *)ntrip->buff,
                         NTRIP_RSP_HTTP))) { /* http response */
    if ((q = strchr(p, '\r')))
      *q = '\0';
    else
      ntrip->buff[128] = '\0';
...
static int waitntrip(ntrip_t *ntrip, char *msg) {
  ...
  if (ntrip->state == 1) { /* read response */
    ...
    if ((n = readtcpcli(ntrip->tcp, (unsigned char *)p,
                        NTRIP_MAXRSP - ntrip->nb - 1, msg)) == 0) {
...
static int statentrip(ntrip_t *ntrip) {
  return !ntrip ? 0
                : (ntrip->state == 0 ? ntrip->tcp->svr.state : ntrip->state);
}
/* get extended state ntrip --------------------------------------------------*/
static int statexntrip(ntrip_t *ntrip, char *msg) {
  char *p = msg;
  int state =
      !ntrip ? 0 : (ntrip->state == 0 ? ntrip->tcp->svr.state : ntrip->state);
...
static void rsp_ntripc_c(ntripc_t *ntripc, int i) {
  ...
  char url[256] = "", mntpnt[256] = "", proto[256] = "", user[256],
       user_pwd[256], *p, *q;
...
/* write ntrip-caster --------------------------------------------------------*/
static int writentripc(ntripc_t *ntripc, unsigned char *buff, int n,
                       char *msg) {
  int i, ns = 0, err;
...
static udp_t *genudp(int type, int port, const char *saddr, char *msg) {
  ...
  if (setsockopt(udp->sock, SOL_SOCKET, SO_RCVBUF, (const char *)&bs,
                 sizeof(bs)) == -1 ||
      setsockopt(udp->sock, SOL_SOCKET, SO_SNDBUF, (const char *)&bs,
                 sizeof(bs)) == -1) {
  ...
  if (!udp->type) { /* udp server */
    ...
    setsockopt(udp->sock, SOL_SOCKET, SO_REUSEADDR, (const char *)&opt,
               sizeof(opt));
#endif
    if (bind(udp->sock, (struct sockaddr *)&udp->addr, sizeof(udp->addr)) ==
        -1) {
...
#endif
{
  ...
  if (ftp->proto == 0) { /* ftp */
    sprintf(opt,
            "--ftp-user=%s --ftp-password=%s --glob=off --passive-ftp %s-t 1 "
            "-T %d -O \"%s\"",
...
/* write memory buffer -------------------------------------------------------*/
static int writemembuf(membuf_t *membuf, unsigned char *buff, int n,
                       char *msg) {
  int i;
...
extern void strinit(stream_t *stream) {
  ...
  stream->tick_i = stream->tick_o = stream->tact = stream->inbt =
      stream->outbt = 0;
 ...
 *(STR_SERIAL,STR_FILE,STR_TCPSVR,...) int mode         I   stream mode
 *(STR_MODE_???) char *path       I   stream path (see below) return : status
 *(0:error,1:ok) notes  : see reference [1] for NTRIP STR_FTP/HTTP needs "wget"
 *in command search paths
 *
 * stream path ([] options):
 *
...
extern int stropen(stream_t *stream, int type, int mode, const char *path) {
  ...
  switch (type) {
    ...
    case STR_NTRIPCLI:
      ...
      break;
    case STR_NTRIPC_S:
      stream->port = openntripc(path, 0, stream->msg);
      break;
    case STR_NTRIPC_C:
      stream->port = openntripc(path, 1, stream->msg);
      break;
    case STR_UDPSVR:
...
extern int strwrite(stream_t *stream, unsigned char *buff, int n) {
  ...
  if ((int)(tick - stream->tick_o) > tirate) {
    stream->outr = (int)((double)(stream->outb - stream->outbt) * 8000 /
                         (tick - stream->tick_o));
    stream->tick_o = tick;
