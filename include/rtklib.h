...
#define miu 3.98600415e14    /* earth gravity constant (m^3/s^2) */
...
#define EFACT_GAL 2.0 /* error factor: Galileo */
#define EFACT_QZS 2.0 /* error factor: QZSS */
#define EFACT_CMP 2.0 /* error factor: BeiDou */
#define EFACT_IRN 3.0 /* error factor: IRNSS */
...
#define NSATQZS 0 /* number of QZSS satellites */
#define NSYSQZS 0
#endif
#ifdef ENACMP
...
#define MINPRNIRN 0
#define MAXPRNIRN 0
#define NSATIRN 0 /* number of IRNSS satellites */
#define NSYSIRN 0
#endif
#ifdef ENALEO
...
#define MINPRNLEO 0
#define MAXPRNLEO 0
#define NSATLEO 0 /* number of LEO satellites */
#define NSYSLEO 0
#endif
#define NSYS                                                   \
  (NSYSGPS + NSYSGLO + NSYSGAL + NSYSQZS + NSYSCMP + NSYSIRN + \
...
#define MAXSAT                                                           \
  (NSATGPS + NSATGLO + NSATGAL + NSATQZS + NSATCMP + NSATIRN + NSATSBS + \
   NSATLEO)
...
#define INT_SWAP_AMB 86400.0  /* swap interval of solution ambiguity file (s) \
                               */
#define INT_SWAP_RES 86400.0  /* swap interval of solution ambigutiy file (s) \
                               */
#define INT_SWAP_AZL 86400.0  /* swap interval of solution ambiguity file (s) \
                               */
...
#define MAXSTRPATH 200    /* max length of stream path */
...
typedef struct {                  /* antenna parameter type */
  ...
  gtime_t sGTimeStart, sGTimeEnd; /* valid time start and end */
...
typedef struct {                  /* satellite fcb data type */
  gtime_t sGTimeStart, sGTimeEnd; /* start/end time (GPST) */
...
typedef struct { /* solution type */
  ...
  /* dtr[0]: receiver clock bias (s) */
  /* dtr[1]: glo-gps time offset (s) */
  /* dtr[2]: gal-gps time offset (s) */
  /* dtr[3]: bds-gps time offset (s) */
  double dtrp;        /* tropsphere deley */
...
typedef struct { /* processing options type */
  TimeCalen_t sTimeStart;
  TimeCalen_t sTimeEnd;
  ...
  double prn[7];   /* process-noise std [0]bias,[1]iono [2]trop [3]acch [4]accv
                      [5] pos [6]isb */
  ...
  int pppadpt;             /* ppp adaptive kalman filter */
  int ppprbst;             /* ppp robust kalman filter */
...
typedef struct { /* solution options type */
  ...
  int samb;      /* solution ambiguity statistics level (0:off,1:on) */
  int sres;      /* solution residuals statistics level (0:off,1:on) */
  int sazl;      /* solution azimuth/elevation angle statistics level (0:off,1:on) */
  int sstat;     /* solution statistics level (0:off,1:states,2:residuals) */
  int trace;     /* debug trace level (0:off,1-6:debug) */

  double nmeaintv[2]; /* nmea output interval (s) (<0:no,0:all) */
  /* nmeaintv[0]:gprmc,gpgga,nmeaintv[1]:gpgsv */
  char sep[64];               /* field separator */
  char prog[64];              /* program name */
  char outposmat[MAXSTRPATH]; /* another solution output for matlab analysis */
...
typedef struct {            /* file options type */
  ...
  char obsfile[MAXSTRPATH];   /* obs file */
  char basefile[MAXSTRPATH];  /* base obs file */
  char navfile[MAXSTRPATH];   /* nav file */
  char orbitfile[MAXSTRPATH]; /* sp3 file */
  char clkfile[MAXSTRPATH];   /* clk file */

  char groundtruth[MAXSTRPATH];   /* ground truth file */
  char pospath[MAXSTRPATH];       /* position file path */
  char pospathmat[MAXSTRPATH];    /* position file for matlab analysis */
  char ambpath[MAXSTRPATH];       /* ambiguity file path */
  char respath[MAXSTRPATH];       /* residual file path */
  char azlpath[MAXSTRPATH];       /* azimuth and elevation angle file path */
  char solstatpath[MAXSTRPATH];   /* solution statistics file path */
  char tracepath[MAXSTRPATH];     /* debug trace file path */
  char file_id[MAXSTRPATH];       /* file id for output file name */
  char prc_id4opname[MAXSTRPATH]; /* process id for output file name */
} filopt_t;

typedef struct {                  /* RINEX options type */
  gtime_t sGTimeStart, sGTimeEnd; /* time start/end */
  double tint;                    /* time interval (s) */
...
typedef struct half_cyc_tag {     /* half-cycle correction list type */
  ...
  gtime_t sGTimeStart, sGTimeEnd; /* time start, time end */
...
typedef struct { /* ecef pnt data point type */
  double tow;
  double xyz[3]; /* point data {x,y,z} (m) */
} ecef_pnt_t;
...
EXPORT double mattrc(double *A, int m, int n);
EXPORT int matinv(double *A, int n);
EXPORT int solve(const char *tr, const double *A, const double *Y, int n, int m,
                 double *X);
EXPORT int lsq(const double *A, const double *y, int n, int m, double *x,
               double *Q);
EXPORT void bubblesort(double *list, int len);
EXPORT int robust(const double *H, const double *Pp, const double *v,
                  const double *xp, int n, int m, const prcopt_t *opt,
                  double *R);
EXPORT int robust2(const double *H, const double *Pp, const double *L,
                   const double *xp, int n, int m, const prcopt_t *opt,
                   double *R);
EXPORT int adapt(const double *H, const double *R, const double *v,
...
EXPORT void imatprint(const int *A, int n, int m, int p, int q);
...
EXPORT int loacaltime(int basetime_hr, double dlon);
...
EXPORT int reppath(const char *path, char *rpath, gtime_t time, const char *rov,
                   const char *base);
EXPORT int reppaths(const char *path, char *rpaths[], int nmax,
...
EXPORT int screent(gtime_t time, gtime_t sGTimeStart, gtime_t sGTimeEnd,
                   double tint);
...
EXPORT int readrnx(const char *file, int rcv, const char *opt, obs_t *obs,
                   nav_t *nav, sta_t *sta);
EXPORT int readrnxt(const char *file, int rcv, gtime_t sGTimeStart,
                    gtime_t sGTimeEnd, double tint, const char *opt, obs_t *obs,
                    nav_t *nav, sta_t *sta);
...
EXPORT int readgt(const char *file);
EXPORT int readsol(char *files[], int nfile, solbuf_t *sol);
EXPORT int readsolt(char *files[], int nfile, gtime_t sGTimeStart,
                    gtime_t sGTimeEnd, double tint, int qflag, solbuf_t *sol);
EXPORT int readsolstat(char *files[], int nfile, solstatbuf_t *statbuf);
EXPORT int readsolstatt(char *files[], int nfile, gtime_t sGTimeStart,
                        gtime_t sGTimeEnd, double tint, solstatbuf_t *statbuf);
EXPORT int inputsol(unsigned char data, gtime_t sGTimeStart, gtime_t sGTimeEnd,
...
EXPORT void outsol(FILE *fp, const sol_t *sol, const double *rb,
                   const solopt_t *opt);
EXPORT void outsolmat(FILE *fp, const sol_t *sol, const double *rb,
                      const solopt_t *opt);
...
EXPORT int convkml(const char *infile, const char *outfile, gtime_t sGTimeStart,
                   gtime_t sGTimeEnd, double tint, int qflg, double *offset,
                   int tcolor, int pcolor, int outalt, int outtime);

/* gpx converter -------------------------------------------------------------*/
EXPORT int convgpx(const char *infile, const char *outfile, gtime_t sGTimeStart,
                   gtime_t sGTimeEnd, double tint, int qflg, double *offset,
                   int outtrk, int outpnt, int outalt, int outtime);

/* sbas functions ------------------------------------------------------------*/
EXPORT int sbsreadmsg(const char *file, int sel, sbs_t *sbs);
EXPORT int sbsreadmsgt(const char *file, int sel, gtime_t sGTimeStart,
                       gtime_t sGTimeEnd, sbs_t *sbs);
...
EXPORT void getsysopts(prcopt_t *sProcessOption, solopt_t *sSolutionOption,
                       filopt_t *sFileOption);
EXPORT void setsysopts(const prcopt_t *sProcessOption,
                       const solopt_t *sSolutionOption,
                       const filopt_t *sFileOption);
...
EXPORT int pntpos(const obsd_t *obs, int *ntemp, const nav_t *nav,
...
EXPORT int rtkopenamb(const char *file, int level);
EXPORT void rtkcloseamb(void);
EXPORT int rtkoutamb(rtk_t *rtk, char *buff);

EXPORT int rtkopenres(const char *file, int level);
EXPORT void rtkcloseres(void);
EXPORT int rtkoutres(rtk_t *rtk, char *buff);

EXPORT int rtkopenazl(const char *file, int level);
EXPORT void rtkcloseazl(void);
EXPORT int rtkoutazl(rtk_t *rtk, char *buff);
...
EXPORT int postpos(gtime_t sGTimeStart, gtime_t sGTimeEnd, double ti, double tu,
                   const prcopt_t *sProcessOption,
                   const solopt_t *sSolutionOption, const filopt_t *sFileOption,
                   char **infile, int n, char *outfile, const char *rov,
                   const char *base);
...
EXPORT void strsvrstat(strsvr_t *svr, int *stat, int *byte, int *bps,
                       char *msg);
...
EXPORT int dl_exec(gtime_t sGTimeStart, gtime_t sGTimeEnd, double ti,
                   int seqnos, int seqnoe, const url_t *urls, int nurl,
                   char **stas, int nsta, const char *dir, const char *usr,
                   const char *pwd, const char *proxy, int opts, char *msg,
                   FILE *fp);
EXPORT void dl_test(gtime_t sGTimeStart, gtime_t sGTimeEnd, double ti,
